...
fun MessagesScreen(modifier: Modifier = Modifier) {
    ...
    NexShapeScreen(
        title = if (selected == null) "Mensagens" else selected?.otherUserName ?: "Conversa",
        subtitle = if (selected == null) "Conversas internas da plataforma." else "Mensagens com suporte ou profissional.",
        modifier = modifier,
        action = {
            Row {
                if (selected != null) {
