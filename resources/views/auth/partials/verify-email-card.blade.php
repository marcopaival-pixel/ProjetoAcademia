...
<div class="bg-zinc-900/40 backdrop-blur-3xl border border-white/5 p-10 rounded-[2.5rem] shadow-2xl text-center">
    <p class="text-zinc-400 text-sm mb-8 leading-relaxed">
        Para acessar as funcionalidades do <strong>{{ config('app.name') }}</strong>, você precisa confirmar que este e-mail lhe pertence. Não recebeu? Verifique sua pasta de spam.
    </p>
    ...
    @else
        <form id="verify-resend-form" method="POST" action="{{ route('verification.resend.guest') }}" class="space-y-6">
            @csrf
            <div class="mb-6">
                <input type="email" name="email" value="{{ request('email') }}" placeholder="Seu e-mail" required
                    class="w-full px-6 py-4 bg-zinc-950 border border-white/10 rounded-2xl text-white text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all outline-none">
            </div>
            <button type="submit" id="verify-resend-submit" class="w-full py-5 bg-blue-600 hover:bg-blue-500 disabled:opacity-60 disabled:pointer-events-none text-white font-black rounded-3xl transition-all active:scale-[0.98] shadow-2xl shadow-blue-600/20 uppercase tracking-[0.2em] text-[10px]">
                Reenviar E-mail de Verificação
            </button>
