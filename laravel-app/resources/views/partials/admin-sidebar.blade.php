    ...
    <div class="sidebar-content flex-1 overflow-y-auto px-4 py-6 admin-sidebar-nav custom-scrollbar" :class="isCollapsed ? 'items-center' : ''">
        ...
        <div class="nav-item mb-1" x-show="isGroupVisible(['Logs', 'Auditoria', 'Erros', 'Backup', 'Deploy'])">
            ...
            <ul class="submenu list-none p-0 m-0 space-y-1 mt-1 pl-4" x-show="openMenus.includes('logs') && !isCollapsed" x-collapse>
                <li x-show="isVisible('Painel de Saúde')"><a href="{{ route('admin.observability.index') }}" class="submenu-link flex items-center px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest {{ request()->routeIs('admin.observability.index') ? 'active' : 'text-zinc-600 hover:text-white' }}">Saúde do Sistema</a></li>
                ...
                <li x-show="isVisible('Histórico Usuário')"><a href="{{ route('admin.observability.user-history') }}" class="submenu-link flex items-center px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest {{ request()->routeIs('admin.observability.user-history*') ? 'active' : 'text-zinc-600 hover:text-white' }}">Histórico de Usuário</a></li>
                <li x-show="isVisible('Métricas APIs')"><a href="{{ route('admin.observability.api-integrations-metrics') }}" class="submenu-link flex items-center px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest {{ request()->routeIs('admin.observability.api-integrations-metrics') ? 'active' : 'text-zinc-600 hover:text-white' }}">Métricas de APIs</a></li>
