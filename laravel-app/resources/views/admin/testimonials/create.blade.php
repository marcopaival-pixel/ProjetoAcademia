...
<div class="max-w-3xl mx-auto space-y-6">
    <div>
        ...
        <h1 class="text-3xl font-black text-white tracking-tight">Novo Depoimento</h1>
    </div>

    <form method="POST" action="{{ route('admin.testimonials.store') }}" enctype="multipart/form-data" class="bg-[#11141b] border border-white/5 p-8 rounded-3xl space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="profession" name="profession" value="{{ old('profession') }}" placeholder="Ex: Personal Trainer, Médico, Gestor"
                       class="w-full bg-zinc-950 border border-white/10 rounded-xl px-4 py-3 text-xs text-white uppercase font-bold tracking-wider placeholder:text-zinc-700 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 transition-all">
        ...
        <div class="grid grid-cols-3 gap-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="state" name="state" value="{{ old('state') }}" max="2" placeholder="Ex: SP"
                       class="w-full bg-zinc-950 border border-white/10 rounded-xl px-4 py-3 text-xs text-white uppercase font-bold tracking-wider placeholder:text-zinc-700 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 transition-all">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-2">
                ...
                <select id="rating" name="rating" required
                    ...
                    <option value="5" {{ old('rating', '5') == '5' ? 'selected' : '' }}>5 Estrelas ★★★★★</option>
                    <option value="4" {{ old('rating') == '4' ? 'selected' : '' }}>4 Estrelas ★★★★</option>
                    <option value="3" {{ old('rating') == '3' ? 'selected' : '' }}>3 Estrelas ★★★</option>
                    <option value="2" {{ old('rating') == '2' ? 'selected' : '' }}>2 Estrelas ★★</option>
                    <option value="1" {{ old('rating') == '1' ? 'selected' : '' }}>1 Estrela ★</option>
                </select>
            ...
            <!-- Avatar -->
            <div class="space-y-2">
                <label for="avatar" class="block text-[10px] font-black uppercase tracking-widest text-zinc-500">Foto do Cliente (Avatar)</label>
                <input type="file" id="avatar" name="avatar" accept="image/*"
                       class="w-full bg-zinc-950 border border-white/10 rounded-xl px-4 py-3 text-xs text-zinc-400 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 transition-all">
                <p class="text-[9px] text-zinc-600 font-bold uppercase tracking-wider mt-1">Formatos sugeridos: JPG, PNG. Tamanho máximo: 1MB.</p>
        ...
        <div class="space-y-2">
            ...
            <textarea id="testimonial" name="testimonial" rows="4" required maxlength="1000" placeholder="Escreva aqui o depoimento ou feedback do cliente..."
                      class="w-full bg-zinc-950 border border-white/10 rounded-xl px-4 py-3 text-xs text-white font-medium placeholder:text-zinc-700 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 transition-all">{{ old('testimonial') }}</textarea>
        ...
        <div class="flex flex-wrap gap-6 pt-4 border-t border-white/5">
            ...
            <label class="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" name="approved" value="1" {{ old('approved', '1') ? 'checked' : '' }}
                       class="w-4 h-4 rounded bg-zinc-950 border-white/10 text-emerald-600 focus:ring-emerald-500 focus:ring-offset-zinc-950">
                <span class="text-[10px] font-black uppercase tracking-widest text-zinc-300">Aprovar Imediatamente</span>
            </label>
        ...
        <div class="flex gap-4 pt-4 justify-end">
            ...
            <button type="submit" class="px-8 py-3.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-black uppercase tracking-wider transition-all shadow-lg shadow-emerald-600/25">Cadastrar Depoimento</button>
        </div>
    </form>
</div>
@endsection
