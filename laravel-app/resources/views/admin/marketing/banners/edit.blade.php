...
<div class="max-w-5xl mx-auto space-y-8 animate__animated animate__fadeIn">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-black text-white tracking-tight">Editar Banner</h1>
                <p class="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mt-1">Atualize as configurações do banner promocional</p>
            </div>
        </div>
        <div class="flex items-center gap-6">
            <div class="text-right">
                <p class="text-[8px] font-black text-zinc-600 uppercase tracking-widest">Visualizações</p>
                <p class="text-lg font-black text-white">{{ $banner->views_count }}</p>
            </div>
            <div class="text-right border-l border-white/5 pl-6">
                <p class="text-[8px] font-black text-zinc-600 uppercase tracking-widest">Cliques</p>
                <p class="text-lg font-black text-emerald-400">{{ $banner->clicks_count }}</p>
            </div>
        </div>
    </div>

    <form action="{{ route('admin.marketing.banners.update', $banner) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('POST') {{-- Note: using POST since update is POST in routes --}}
                ...
                <div class="bg-zinc-900/50 border border-white/5 p-8 rounded-[2.5rem] backdrop-blur-xl space-y-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title', $banner->title) }}" required class="w-full bg-zinc-950 border border-white/5 rounded-2xl px-6 py-4 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-600/50 transition-all" placeholder="Ex: Grande Lançamento NexShape App">
                        </div>

                        <div>
                            <label class="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1 mb-2 block">Subtítulo</label>
                            <input type="text" name="subtitle" value="{{ old('subtitle', $banner->subtitle) }}" class="w-full bg-zinc-950 border border-white/5 rounded-2xl px-6 py-4 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-600/50 transition-all" placeholder="Ex: Disponível agora nas lojas">
                        </div>

                        <div>
                            <label class="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1 mb-2 block">Descrição</label>
                            <textarea name="description" rows="4" class="w-full bg-zinc-950 border border-white/5 rounded-2xl px-6 py-4 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-600/50 transition-all resize-none" placeholder="Conte mais detalhes sobre a promoção...">{{ old('description', $banner->description) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                        <div>
                            ...
                            <div class="flex items-center gap-3 bg-zinc-950 border border-white/5 rounded-2xl px-4 py-2">
                                <input type="color" name="background_color" value="{{ old('background_color', $banner->background_color) }}" class="w-10 h-10 bg-transparent border-none cursor-pointer">
                                <input type="text" value="{{ old('background_color', $banner->background_color) }}" readonly class="bg-transparent border-none text-zinc-400 text-xs font-mono">
                            </div>
                        </div>
                        <div>
                            <label class="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1 mb-2 block">Ícone (FontAwesome)</label>
                            <input type="text" name="icon" value="{{ old('icon', $banner->icon) }}" class="w-full bg-zinc-950 border border-white/5 rounded-2xl px-6 py-4 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-600/50 transition-all" placeholder="Ex: fas fa-rocket">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-zinc-900/50 border border-white/5 p-8 rounded-[2.5rem] backdrop-blur-xl space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            ...
                            <input type="text" name="primary_button_text" value="{{ old('primary_button_text', $banner->primary_button_text) }}" class="w-full bg-zinc-950 border border-white/5 rounded-2xl px-6 py-4 text-white text-sm" placeholder="Texto do botão">
                            <input type="text" name="primary_button_link" value="{{ old('primary_button_link', $banner->primary_button_link) }}" class="w-full bg-zinc-950 border border-white/5 rounded-2xl px-6 py-4 text-white text-sm" placeholder="URL do botão">
                        </div>
                        <div class="space-y-4">
                            <p class="text-[9px] font-black text-zinc-600 uppercase tracking-[0.2em]">Botão Secundário</p>
                            <input type="text" name="secondary_button_text" value="{{ old('secondary_button_text', $banner->secondary_button_text) }}" class="w-full bg-zinc-950 border border-white/5 rounded-2xl px-6 py-4 text-white text-sm" placeholder="Texto do botão">
                            <input type="text" name="secondary_button_link" value="{{ old('secondary_button_link', $banner->secondary_button_link) }}" class="w-full bg-zinc-950 border border-white/5 rounded-2xl px-6 py-4 text-white text-sm" placeholder="URL do botão">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-zinc-900/50 border border-white/5 p-8 rounded-[2.5rem] backdrop-blur-xl space-y-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <div class="bg-zinc-950 border border-white/5 rounded-2xl p-4 max-h-48 overflow-y-auto space-y-2 custom-scrollbar">
                                @php $bannerRoles = $banner->roles->pluck('id')->toArray(); @endphp
                                @foreach($roles as $role)
                                <label class="flex items-center gap-3 p-2 hover:bg-white/[0.02] rounded-xl cursor-pointer transition-all">
                                    <input type="checkbox" name="roles[]" value="{{ $role->id }}" {{ in_array($role->id, $bannerRoles) ? 'checked' : '' }} class="w-4 h-4 rounded border-white/10 bg-zinc-900 text-blue-600 focus:ring-offset-zinc-950">
                        ...
                        <div>
                            ...
                            <input type="number" name="priority" value="{{ old('priority', $banner->priority) }}" class="w-full bg-zinc-950 border border-white/5 rounded-2xl px-6 py-4 text-white text-sm" placeholder="0">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="display_type" class="w-full bg-zinc-950 border border-white/5 rounded-2xl px-6 py-4 text-white text-sm appearance-none">
                                <option value="always" {{ old('display_type', $banner->display_type) == 'always' ? 'selected' : '' }}>Sempre exibir</option>
                                <option value="once" {{ old('display_type', $banner->display_type) == 'once' ? 'selected' : '' }}>Apenas uma vez</option>
                                <option value="until_closed" {{ old('display_type', $banner->display_type) == 'until_closed' ? 'selected' : '' }}>Até o usuário fechar</option>
                                <option value="frequency" {{ old('display_type', $banner->display_type) == 'frequency' ? 'selected' : '' }}>Frequência Customizada</option>
                            </select>
                        </div>

                        <div id="frequency_days_container" class="{{ old('display_type', $banner->display_type) == 'frequency' ? '' : 'hidden' }}">
                            <label class="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1 mb-2 block">Reaparecer após (dias)</label>
                            <input type="number" name="frequency_days" value="{{ old('frequency_days', $banner->frequency_days) }}" class="w-full bg-zinc-950 border border-white/5 rounded-2xl px-6 py-4 text-white text-sm" placeholder="7">
                        </div>
                    </div>
                    ...
                    <div class="pt-4 space-y-3 border-t border-white/5">
                        <label class="flex items-center justify-between p-2 cursor-pointer group">
                            ...
                            <input type="checkbox" name="is_active" value="1" {{ $banner->is_active ? 'checked' : '' }} class="w-10 h-5 rounded-full border-white/10 bg-zinc-900 text-blue-600 focus:ring-offset-zinc-950">
                        </label>
                        <label class="flex items-center justify-between p-2 cursor-pointer group">
                            ...
                            <input type="checkbox" name="allow_dismiss" value="1" {{ $banner->allow_dismiss ? 'checked' : '' }} class="w-10 h-5 rounded-full border-white/10 bg-zinc-900 text-blue-600 focus:ring-offset-zinc-950">
                        </label>
                        <label class="flex items-center justify-between p-2 cursor-pointer group">
                            ...
                            <input type="checkbox" name="dont_show_again_option" value="1" {{ $banner->dont_show_again_option ? 'checked' : '' }} class="w-10 h-5 rounded-full border-white/10 bg-zinc-900 text-blue-600 focus:ring-offset-zinc-950">
                        </label>
                    </div>
                </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            @if($banner->image_desktop)
                                <img src="{{ $banner->image_desktop }}" class="w-full h-32 object-cover rounded-2xl mb-4 border border-white/10">
                            @endif
                        ...
                        <div>
                            ...
                            @if($banner->image_mobile)
                                <img src="{{ $banner->image_mobile }}" class="w-full h-32 object-cover rounded-2xl mb-4 border border-white/10">
                            @endif
        ...
        <div class="flex items-center justify-end gap-4 pt-8">
            ...
            <button type="submit" class="px-12 py-4 bg-blue-600 text-white font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-blue-500 transition-all shadow-xl shadow-blue-600/20 active:scale-95">Salvar Alterações</button>
        </div>
    </form>
</div>
