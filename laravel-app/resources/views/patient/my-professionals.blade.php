...
<div class="py-12 space-y-12 animate-fade-in max-w-[1200px] mx-auto px-6">
    <div class="text-center space-y-4">
        ...
        <p class="text-zinc-500 font-medium text-lg">Gerencie o acesso aos seus dados de saude por cada especialista.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        @forelse($links as $link)
            <div class="bg-zinc-900/40 backdrop-blur-xl border border-white/5 rounded-[3rem] p-8 space-y-8 shadow-2xl">
                ...
                @php
                    ...
                    $modules = [
                        'view_assessments' => ['icon' => 'clipboard-check', 'label' => 'Ver avaliacoes fisicas', 'sensitive' => false],
                        'view_diets' => ['icon' => 'utensils', 'label' => 'Ver planos alimentares', 'sensitive' => false],
                        'view_workouts' => ['icon' => 'dumbbell', 'label' => 'Ver fichas de treino', 'sensitive' => false],
                        'view_medical_records' => ['icon' => 'file-medical', 'label' => 'Ver prontuario medico', 'sensitive' => false],
                        'view_evolution' => ['icon' => 'line-chart', 'label' => 'Ver graficos de evolucao', 'sensitive' => false],
                        'psychology_session_notes' => ['icon' => 'lock', 'label' => 'Autorizar registros psicologicos', 'sensitive' => true],
                        'restricted_notes' => ['icon' => 'user-shield', 'label' => 'Autorizar observacoes restritas', 'sensitive' => true],
                    ];
                @endphp
                ...
                <form action="{{ route('patient.my-professionals.update-permissions', $link->id) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            @foreach($modules as $key => $module)
                                @php
                                    $checked = $module['sensitive']
                                        ? (bool) ($perms[$key] ?? false)
                                        : (bool) ($perms[$key] ?? true);
                                @endphp
                                ...
                                <label class="flex items-center justify-between p-4 bg-black/20 rounded-2xl border border-white/5 cursor-pointer hover:bg-black/40 transition-colors">
                                    <div class="flex items-center gap-4">
                                        <div class="w-10 h-10 bg-zinc-800 rounded-xl flex items-center justify-center {{ $module['sensitive'] ? 'text-rose-400' : 'text-zinc-400' }}">
                                            <i class="fas fa-{{ $module['icon'] }}"></i>
                                        </div>
                                        <div>
                                            <span class="text-white text-sm font-bold">{{ $module['label'] }}</span>
                                            @if($module['sensitive'])
                                                <p class="text-rose-300/70 text-[9px] font-black uppercase tracking-widest mt-1">Dado sensivel</p>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" name="permissions[{{ $key }}]" value="1" class="sr-only peer" {{ $checked ? 'checked' : '' }}>
                    ...
                    <button type="submit" class="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black uppercase text-[10px] tracking-widest rounded-2xl transition-all">
                        Salvar Permissoes
                    </button>
                </form>

                <form action="{{ route('patient.my-professionals.revoke', $link->id) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja revogar o acesso deste profissional?');">
                    @csrf
        ...
        @empty
            <div class="col-span-full py-20 text-center bg-zinc-900/20 border border-white/5 border-dashed rounded-[3rem]">
                ...
                <p class="text-zinc-500 font-bold mb-6">Voce ainda nao concedeu acesso a nenhum especialista.</p>
