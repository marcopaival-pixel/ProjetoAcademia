...
<div class="py-10 space-y-12 animate-dashboard-entry max-w-4xl mx-auto px-6">
    ...
    <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-8 pb-4 border-b border-white/5">
        <div class="space-y-3">
            <h1 class="text-4xl font-black tracking-tight text-white leading-tight">
                Novo <span class="bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-indigo-400">Lançamento</span>
            </h1>
        </div>
    ...
    <form action="{{ route('professional.finance.entries.store') }}" method="POST" class="bg-zinc-900/60 backdrop-blur-2xl border border-white/10 p-10 rounded-[3rem] shadow-2xl space-y-8" x-data="{ type: 'revenue', status: 'pending' }">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="space-y-2 md:col-span-2">
                ...
                <input type="text" name="description" value="{{ old('description') }}" required class="w-full bg-zinc-950/50 border border-white/5 rounded-2xl p-4 text-white text-sm font-bold focus:ring-2 focus:ring-blue-500/50 outline-none transition-all" placeholder="Ex: Mensalidade, Compra de Equipamento...">
            </div>

            <div class="space-y-2">
                <label class="text-[10px] font-black uppercase tracking-widest text-zinc-500 px-2">Valor (R$)</label>
                <input type="number" step="0.01" name="amount" value="{{ old('amount') }}" required class="w-full bg-zinc-950/50 border border-white/5 rounded-2xl p-4 text-white text-sm font-bold focus:ring-2 focus:ring-blue-500/50 outline-none transition-all">
            </div>
            ...
            <div class="space-y-2">
                ...
                <select name="category_id" class="w-full bg-zinc-950/50 border border-white/5 rounded-2xl p-4 text-white text-sm font-bold focus:ring-2 focus:ring-blue-500/50 outline-none transition-all">
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" x-show="type === '{{ $category->type }}'">{{ $category->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="date" name="due_date" value="{{ old('due_date', date('Y-m-d')) }}" required class="w-full bg-zinc-950/50 border border-white/5 rounded-2xl p-4 text-white text-sm font-bold focus:ring-2 focus:ring-blue-500/50 outline-none transition-all">
            </div>
            ...
            <div class="space-y-2" x-show="status === 'paid'">
                ...
                <input type="date" name="payment_date" value="{{ old('payment_date') }}" class="w-full bg-zinc-950/50 border border-white/5 rounded-2xl p-4 text-white text-sm font-bold focus:ring-2 focus:ring-blue-500/50 outline-none transition-all">
            </div>

            <div class="space-y-2" x-show="status === 'paid'">
                <label class="text-[10px] font-black uppercase tracking-widest text-zinc-500 px-2">Forma de Pagamento</label>
                <input type="text" name="payment_method" value="{{ old('payment_method') }}" class="w-full bg-zinc-950/50 border border-white/5 rounded-2xl p-4 text-white text-sm font-bold focus:ring-2 focus:ring-blue-500/50 outline-none transition-all" placeholder="Ex: PIX, Cartão de Crédito...">
            </div>

            <div class="space-y-2 md:col-span-2">
                <label class="text-[10px] font-black uppercase tracking-widest text-zinc-500 px-2">Observações</label>
                <textarea name="notes" rows="3" class="w-full bg-zinc-950/50 border border-white/5 rounded-2xl p-4 text-white text-sm font-bold focus:ring-2 focus:ring-blue-500/50 outline-none transition-all">{{ old('notes') }}</textarea>
            </div>
        </div>
