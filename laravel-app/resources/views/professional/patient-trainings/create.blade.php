...
<div x-data="trainingPlanBuilder()" 
    ...
    <header class="pb-8 border-b border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        <div class="relative z-10">
            <a href="{{ route('professional.patients.trainings.index', $patient->id) }}" class="px-4 py-2 bg-zinc-800/50 border border-white/5 text-zinc-300 rounded-xl hover:bg-zinc-800 transition-all text-xs font-bold inline-flex items-center gap-2 mb-6">
                <i class="fas fa-chevron-left"></i> Voltar aos Treinos de {{ $patient->name }}
            </a>
            <h1 class="text-4xl font-black text-white tracking-tight">Criar Treino <span class="text-blue-500 text-3xl opacity-50 block mt-1">{{ $patient->name }}</span></h1>
    ...
    <div class="bg-zinc-900/40 backdrop-blur-3xl border border-white/5 rounded-[3rem] p-10 shadow-2xl relative overflow-hidden group/card">
        ...
        <form action="{{ route('professional.patients.trainings.store', $patient->id) }}" method="POST" id="planForm" @submit.prevent="submitForm()" class="space-y-12 relative z-10">
            @csrf
            ...
            <div x-show="step === 1" x-transition.opacity.duration.400ms class="space-y-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <!-- Perfil do {{ $patientLabel }} -->
                    <div class="space-y-3">
                        <label class="block text-[10px] text-zinc-500 font-black uppercase tracking-widest ml-2">Perfil do {{ $patientLabel }}</label>
                        <div class="relative group/select">
