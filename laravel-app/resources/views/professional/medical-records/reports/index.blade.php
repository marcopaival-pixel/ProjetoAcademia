            ...
            <div class="bg-zinc-900 border border-zinc-800 rounded-[2rem] p-6 hover:border-blue-500/30 transition-all group">
                <div class="flex justify-between items-start mb-4">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button x-data @click="$dispatch('open-modal', 'edit-report-{{ $report->id }}')" class="p-2 bg-zinc-800 text-zinc-400 rounded-lg hover:text-white transition-all"><i class="fas fa-pen"></i></button>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between pt-4 border-t border-zinc-800">
                    ...
                    <form method="POST" action="{{ route('professional.patients.medical-records.reports.destroy', [$patient->id, $report->id]) }}" onsubmit="return confirm('Remover este laudo?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="text-red-400 text-xs font-bold hover:underline">Remover</button>
                    </form>
                </div>
            </div>

            <x-modal name="edit-report-{{ $report->id }}" focusable>
                <form method="POST" action="{{ route('professional.patients.medical-records.reports.update', [$patient->id, $report->id]) }}" class="p-8">
                    @csrf
                    @method('PUT')
                    <h2 class="text-2xl font-black text-white mb-6">Editar Laudo</h2>
                    <div class="space-y-6">
                        <input type="text" name="title" value="{{ $report->title }}" required class="w-full bg-zinc-800 border-none rounded-xl py-3 px-4 text-white focus:ring-2 focus:ring-blue-500">
                        <input type="date" name="date" value="{{ $report->date?->format('Y-m-d') }}" required class="w-full bg-zinc-800 border-none rounded-xl py-3 px-4 text-white focus:ring-2 focus:ring-blue-500">
                        <textarea name="description" rows="4" class="w-full bg-zinc-800 border-none rounded-xl py-3 px-4 text-white focus:ring-2 focus:ring-blue-500">{{ $report->description }}</textarea>
                        <textarea name="conclusion" rows="2" class="w-full bg-zinc-800 border-none rounded-xl py-3 px-4 text-white focus:ring-2 focus:ring-blue-500">{{ $report->conclusion }}</textarea>
                        <textarea name="observations" rows="2" class="w-full bg-zinc-800 border-none rounded-xl py-3 px-4 text-white focus:ring-2 focus:ring-blue-500">{{ $report->observations }}</textarea>
                    </div>
