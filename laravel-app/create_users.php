...
if (!User::where('email', 'aluno@academia.com')->exists()) {
    $u1 = new User([
        'name' => 'Aluno Teste',
...
if (!User::where('email', 'paciente@academia.com')->exists()) {
    $u2 = new User([
        'name' => 'Paciente Teste',
    ...
    $u2->save();
    $u2->roles()->attach($alunoRole->id);
    UserProfile::create(['user_id' => $u2->id, 'birth_date' => '1995-01-01', 'sex' => 'F']);
}

if (!User::where('email', 'profissional@academia.com')->exists()) {
    $u3 = new User([
