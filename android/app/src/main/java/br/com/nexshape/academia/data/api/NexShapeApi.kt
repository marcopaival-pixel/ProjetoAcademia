...
interface NexShapeApi {
    ...
    @Multipart
    @POST("training-plans")
    suspend fun createTrainingPlanMultipart(
        @Part("name") name: RequestBody,
        @Part("plan_label") planLabel: RequestBody?,
        @Part("goal") goal: RequestBody?,
        @Part("description") description: RequestBody?,
        @Part("frequency") frequency: RequestBody?,
        @Part("difficulty") difficulty: RequestBody?,
        @Part("estimated_duration") estimatedDuration: RequestBody?,
        @Part("student_profile") studentProfile: RequestBody?,
        @Part("split_type") splitType: RequestBody?,
        @Part("status") status: RequestBody?,
        @Part("days_of_week") daysOfWeek: RequestBody?,
        @Part("total_volume") totalVolume: RequestBody?,
        @Part("muscles_worked") musclesWorked: RequestBody?,
        @Part("target_areas") targetAreas: RequestBody?,
        @Part("is_template") isTemplate: RequestBody?,
        @Part("exercises") exercises: RequestBody,
        @Part targetPhotos: List<MultipartBody.Part>,
    ...
    @Multipart
    @POST("workout-import/validate")
    suspend fun validateWorkoutImport(
        @Part photo: MultipartBody.Part,
    ): ApiSuccessResponse<WorkoutValidationData>
    ...
    @Multipart
    @POST("workout-import/orchestrated/initialize")
    suspend fun initializeWorkoutImportOrchestrated(
        @Part photos: List<MultipartBody.Part>,
    ): ApiSuccessResponse<WorkoutOrchestratedImportData>

    @POST("workout-import/orchestrated/validate/{uuid}")
    suspend fun validateWorkoutImportOrchestrated(
        @Path("uuid") uuid: String,
    ): ApiSuccessResponse<WorkoutOrchestratedImportData>

    @POST("workout-import/orchestrated/process/{uuid}")
    suspend fun processWorkoutImportOrchestrated(
        @Path("uuid") uuid: String,
    ): ApiSuccessResponse<WorkoutOrchestratedImportData>

    @GET("workout-import/orchestrated/status/{uuid}")
    suspend fun workoutImportOrchestratedStatus(
        @Path("uuid") uuid: String,
    ): ApiSuccessResponse<WorkoutOrchestratedImportData>

    @Multipart
    @POST("workout-import/orchestrated/substitute/{uuid}")
    suspend fun substituteWorkoutImportOrchestrated(
        @Path("uuid") uuid: String,
    ...
    suspend fun analyzeMeal(@Body body: AnalyzeMealRequest): ApiSuccessResponse<AnalyzeMealData>

    @Multipart
    @POST("nutrition/analyze-photo")
    suspend fun analyzeNutritionPhoto(
        @Part photo: MultipartBody.Part,
    ...
    suspend fun evolutionReport(): ApiSuccessResponse<EvolutionReportDto>

    @GET("evolution-reports/consent")
    suspend fun evolutionReportConsent(): ApiSuccessResponse<EvolutionReportConsentDto>

    @POST("evolution-reports")
    suspend fun requestEvolutionReport(@Body body: EvolutionReportRequest): ApiSuccessResponse<EvolutionReportRequestData>

    @GET("evolution-reports/{id}")
    suspend fun evolutionReportStatus(@Path("id") id: Int): ApiSuccessResponse<EvolutionReportStatusData>

    @Multipart
    @POST("evolution-photos/validate")
    suspend fun validateEvolutionPhoto(
        @Part photo: MultipartBody.Part,
    ): ApiSuccessResponse<BodyPhotoValidationDto>

    @POST("evolution-photos/analyze-session")
    suspend fun analyzeEvolutionSession(@Body body: EvolutionSessionAnalyzeRequest): ApiSuccessResponse<EvolutionSessionAnalysisData>
    ...
    @GET("body-analysis")
    suspend fun bodyAnalyses(): ApiSuccessResponse<BodyAnalysisListData>

    @GET("body-analysis/{id}")
    suspend fun bodyAnalysis(@Path("id") id: Int): ApiSuccessResponse<BodyAnalysisData>

    @GET("body-analysis/compare")
    suspend fun compareBodyAnalysis(
        @Query("id1") id1: Int,
        @Query("id2") id2: Int,
    ): ApiSuccessResponse<BodyAnalysisCompareData>
    ...
    suspend fun uploadBodyAnalysisPhoto(
        ...
        @Part("landmarks") landmarks: RequestBody? = null,
        @Part("metrics") metrics: RequestBody? = null,
    ): ApiSuccessResponse<BodyAnalysisData>
