...
Cliente nativo Kotlin + Jetpack Compose para a API REST v1 (`backend`).
...
1. Android Studio → **Open** → pasta `android/`
...
|----------|-------------|
| Emulador + XAMPP | `http://10.0.2.2/NexShape/backend/public/api/v1/` |
| Emulador + `artisan serve` | `http://10.0.2.2:8000/api/v1/` |
| Dispositivo físico (LAN) + XAMPP | `http://192.168.x.x/NexShape/backend/public/api/v1/` |
...
1. Subir Apache/XAMPP com `backend/public` acessível
...
Ver `../backend/docs/API_V1.md` e `openapi-v1.yaml`.
