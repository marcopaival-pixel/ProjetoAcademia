...
Estado do app Android **Aluno + Profissional** alinhado à API v1 (`backend`). Clínica permanece no painel web.
...
1. **Backend** em HTTPS com `backend/public` como document root
2. **`FCM_SERVER_KEY`** no `.env` Laravel
3. **`google-services.json`** em `android/app/`
...
GitHub Actions (`.github/workflows/android-ci.yml`): `assembleDebug` + `lintDebug` em alterações em `android/`.
