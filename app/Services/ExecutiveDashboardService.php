...
class ExecutiveDashboardService
{
    ...
    public function getDashboardData(): array
    {
        return Cache::remember($this->cacheKey(), 300, function () {
            ...
            $thisMonthRevenue = MercadoPagoCredit::whereMonth('created_at', now()->month)
                ->whereYear('created_at', now()->year)
                ->sum('transaction_amount');

            $lastMonthRevenue = MercadoPagoCredit::whereMonth('created_at', now()->subMonth()->month)
                ->whereYear('created_at', now()->subMonth()->year)
                ->sum('transaction_amount');

            $annualRevenue = MercadoPagoCredit::whereYear('created_at', now()->year)
                ->sum('transaction_amount');
            ...
            $delinquentCount = Subscription::whereIn('status', [
                Subscription::FIN_ATRASADO,
                Subscription::STATUS_OVERDUE,
            ])->count();

            $cancellationsThisMonth = Subscription::whereIn('status', [
