...
**Subir:** `backend` (código + migrações + testes + exemplos `.env`) + `android` (código + documentação) + workflows de CI + documentação de deploy/monitoramento.  
...
|-------------------|--------|
| `backend/app/` | Código da aplicação (controllers, models, services, middleware) |
| `backend/routes/` | Rotas web + API v1 |
| `backend/config/` | Configuração Laravel |
| `backend/database/migrations/` | Esquema da base de dados |
| `backend/database/seeders/` | Dados iniciais (RBAC, homolog bootstrap, etc.) |
| `backend/resources/` | Views Blade, JS/CSS fonte |
| `backend/public/` (exceto `build/` gerado) | `index.php`, `.htaccess`, assets estáticos |
| `backend/bootstrap/` | Bootstrap da aplicação |
| `backend/composer.json` + `composer.lock` | Dependências PHP |
| `backend/package.json` + lockfile | Build Vite do front |
| `backend/vite.config.js` (e ficheiros Vite relacionados) | Compilação do front no deploy |
| `backend/.env.example` | Modelo de ambiente (dev) |
| `backend/.env.example.production` | Modelo de ambiente (produção) |
| `backend/phpunit.xml`, `phpstan.neon.dist`, `phpstan-baseline.neon` | Testes e análise estática |
| `backend/tests/` | Suíte automatizada (PHPUnit) |
| `backend/VERSION` (se existir) | Versão para deploy/checklist |
| `backend/artisan` | CLI Laravel |

**Correções recentes (junho/2026) — incluir no commit se ainda não versionadas:**

- `backend/app/Services/PanelAccessService.php`
- `backend/app/Services/MenuAccessService.php`
- `backend/app/Http/Middleware/CheckRole.php`
- `backend/routes/patient.php`
- `backend/app/Services/CommissionClawbackService.php`
- `backend/app/Services/AI/OrchestratorService.php`
- `backend/app/Services/Operations/SystemHealthService.php`
- `backend/tests/Feature/CommissionClawbackServiceTest.php` (e demais testes alterados)

### 1.2 API mobile + app Android

| Pasta / ficheiros | Porquê |
|-------------------|--------|
| `android/app/` | Código Kotlin (Compose, API v1, FCM, etc.) |
| `android/build.gradle.kts`, `settings.gradle.kts` | Configuração Gradle |
| `android/gradle/`, `gradlew`, `gradlew.bat` | Wrapper Gradle |
| `android/gradle.properties`, `gradle/libs.versions.toml` | Versões e propriedades |
| `android/keystore.properties.example` | Exemplo de assinatura (não o ficheiro real) |
| `android/.gitignore` | Exclusão de segredos Android |
| `android/README.md` | Documentação do app |
| `android/RELEASE_BUILD.md`, `RELEASE_CANDIDATE.md` | Build e RC |
| `android/MANUAL_TEST_PLAN.md` | Testes manuais |
| `android/PLAY_STORE_*.md`, `PRIVACY_POLICY_APP.md` | Play Store |
...
| `backend/.github/workflows/laravel-ci.yml` | CI no subprojeto Laravel |
| `backend/.github/PULL_REQUEST_TEMPLATE.md` | Disciplina de PR |
| `scripts/prepare-deploy-package.ps1` (raiz, se existir) | Pacote ZIP de deploy |
| `backend/scripts/staging-release.ps1` | Release em homologação |
| `backend/scripts/setup-dev.ps1` | Setup dev local |
| `backend/deploy.bat`, `optimize_server.sh` (se existirem) | Deploy/otimização servidor |
...
| `backend/docs/GO_LIVE_CHECKLIST.md` | Fases A–D go-live |
| `backend/docs/DEPLOY.md` | Fluxo de branches e painel deploy |
...
| Ficheiros | Porquê |
|-----------|--------|
| `.gitignore` (raiz, `backend/`, `android/`) | Proteção de segredos e artefactos |
...
| Item | Motivo |
|------|--------|
| `backend/.env` | Segredos reais (BD, MP, OpenAI, etc.) |
| `backend/.env.production` **com valores preenchidos** | Segredos de produção |
| `backend/vendor/` | Gerado com `composer install` |
| `backend/node_modules/` | Gerado com `npm ci` |
| `backend/public/build/` | Gerado com `npm run build` — enviar no **deploy**, não no Git |
| `backend/public/hot` | Dev Vite |
| `backend/storage/logs/`, cache, sessões, `framework/views` compiladas | Runtime local |
| `backend/.phpunit.result.cache`, `storage/phpstan/` | Cache de ferramentas |
| `backend/auth.json` com tokens | Credenciais Composer |
| `android/google-services.json` | Config Firebase (segredo do projeto) |
| `android/keystore.properties` | Senhas do keystore |
| `android/*.jks`, `*.keystore` | Chaves de assinatura Play Store |
| `android/build/`, `*.apk`, `*.aab` | Artefactos de build |
...
| `backend/ - Copia.env`, `.env copy.example` | Cópias de ambiente |
| `brain/`, `backend/brain/`, `**/scratch/` | Rascunhos / artefactos IA |
...
- Raiz: `docs/CHAVE_GOOGLE.txt`, `brain/`, `backend/storage/phpstan/`
- `backend/`: `.env`, `vendor/`, `node_modules/`, `public/build`
- `android/`: `google-services.json`, `keystore.properties`, `*.jks`
...
| **`web/`** | Template React/Vite isolado. O front ativo está em `backend` (Vite integrado). |
...
| **`backend/Fabrica/`** | Fábrica de agentes Cursor (auditoria/evolução), não é a app. |
| **`ai-agents/`** | Especificações PRD de agentes. |
...
| **`package-lock.json` na raiz** | Provável órfão — o Node do projeto está em `backend/`. |
...
| **`backend/Fabrica/.github/workflows/validate.yml`** | CI da fábrica de agentes, não da app. |
...
- Quase todo o diretório **`android/`** (app v1.8.1, API v1, FCM, ProGuard).
- Dezenas de ficheiros em **`backend/app/`**:
...
- **`backend/database/migrations/`** novas (2026).
- **`backend/app/Console/Commands/`** novos (audit, release, finance, etc.).
- **`backend/tests/Feature/ApiV1*`**, `AuditRemediationTest`, `ReleaseSmokeTest`, etc.
...
- `backend/docs/GO_LIVE_CHECKLIST.md`
- `backend/database/seeders/DeployHomologBootstrapSeeder.php`
- `backend/app/Models/DeployRelease.php` e controladores/policies de deploy
...
| Passo | Detalhe |
|-------|---------|
| 1. `.env` real | Copiar de `backend/.env.example.production` e preencher |
...
```
NexShape/
├── backend/          ← APP PRINCIPAL (obrigatório)
├── android/          ← App mobile (obrigatório se usar Play Store)
...
- Go-live: `backend/docs/GO_LIVE_CHECKLIST.md`
- Android release: `android/RELEASE_BUILD.md`, `RELEASE_CANDIDATE.md`
- Auditoria readiness: `docs/AUDITORIA_360_2026-05-21.md`
- `.gitignore` raiz: `/.gitignore`
- `.gitignore` Laravel: `backend/.gitignore`
- `.gitignore` Android: `android/.gitignore`
