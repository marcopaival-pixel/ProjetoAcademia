...
class NutritionController extends Controller
{
    ...
    public function processPhoto(Request $request, \App\Services\AI\OrchestratorService $orchestrator)
    {
        ...
        $prompt = implode("\n", [
            "Analise esta foto de refeicao.",
            "Primeiro confirme se a imagem mostra comida/refeicao.",
            "Se nao for comida, responda document_type unknown_document.",
            "Se for refeicao, identifique alimentos, estime quantidades e macros.",
            "Ao final, retorne uma lista JSON de alimentos no formato:",
        ...
        if ($result['status'] === 'success') {
            if (($result['intent'] ?? null) === 'unknown_document') {
                return response()->json([
                    'success' => false,
                    'code' => 'not_food',
                    'error' => 'A imagem nao foi reconhecida como uma refeicao. Nenhum registro foi criado.',
                ], 422);
            }
            ...
            if ($image) {
                ...
                for ($x = 0; $x < $width; $x += $stepX) {
                    for ($y = 0; $y < $height; $y += $stepY) {
                        ...
                        $b = $rgb & 0xFF;
                        $brightness = ($r + $g + $b) / 3;
                        if ($brightness < 25) $dark++;
                        if ($brightness > 245) $bright++;
                        $colorBuckets[(int)($r / 32) . '-' . (int)($g / 32) . '-' . (int)($b / 32)] = true;
                        $sample++;
                    }
                }
                imagedestroy($image);

                if ($sample > 0 && ($dark / $sample) > 0.82) {
                    return ['ok' => false, 'message' => 'A foto esta escura demais para validar alimento.'];
                }

                if ($sample > 0 && ($bright / $sample) > 0.86) {
                    return ['ok' => false, 'message' => 'A foto esta clara demais para validar alimento.'];
                }

                if (count($colorBuckets) < 6) {
                    return ['ok' => false, 'message' => 'A imagem parece ter pouco detalhe visual para uma refeicao.'];
                }
            }
        }
