...
class TrainingPlanController extends Controller
{
    ...
    public function storeTargetSelection(Request $request)
    {
        ...
        if ($request->hasFile('photo')) {
            $photoValidation = app(BodyPhotoValidationService::class)->validate(Auth::user(), $request->file('photo'));
            if (!($photoValidation['approved'] ?? false)) {
                return back()->withErrors(['photo' => implode(' ', $photoValidation['messages'] ?? ['A foto enviada não foi aprovada.'])]);
            }
        ...
        foreach ((array) $request->file('photos', []) as $type => $photo) {
            if (!$photo) {
                continue;
            }

            $validation = app(BodyPhotoValidationService::class)->validate(Auth::user(), $photo);
            if (!($validation['approved'] ?? false)) {
                return back()->withErrors(['photos' => implode(' ', $validation['messages'] ?? ['Uma das fotos enviadas não foi aprovada.'])]);
            }
        ...
        session([
            ...
            'body_target_photo_validation' => $photoValidation,
            'body_target_photos' => $storedPhotos,
        ]);

        return redirect()->route('progression.plans.create');
    }

    public function validateTargetPhoto(Request $request, BodyPhotoValidationService $validator)
    {
        $request->validate([
    ...
    public function store(Request $request)
    {
        ...
        $photoValidation = session('body_target_photo_validation', null);
        $bodyTargetPhotos = session('body_target_photos', []);

        $this->storeBodyTargetEvolutionPhotos($bodyTargetPhotos, $plan);
        ...
        if (!empty($selectedTargets)) {
            foreach ($selectedTargets as $target) {
                ...
                \App\Models\WorkoutTargetArea::create([
                    ...
                    'photo_validation_status' => $photoValidation['status'] ?? null,
                    'photo_validation_data' => $photoValidation ? array_merge($photoValidation, [
                        'photos' => $bodyTargetPhotos,
                    ]) : null,
                ]);
            }
            session()->forget(['selected_body_targets', 'body_target_photo', 'body_target_photo_validation', 'body_target_photos']);
        }

        return redirect()->route('progression.plans.index')->with('success', 'Plano de treino criado com sucesso!');
    }

    private function storeBodyTargetEvolutionPhotos(array $photos, TrainingPlan $plan): void
    {
        foreach ($photos as $photo) {
            ...
            \App\Models\EvolutionPhoto::firstOrCreate(
                [
                    'user_id' => Auth::id(),
                    'photo_path' => $path,
                ],
                [
                    ...
                    'notes' => json_encode([
                        'source' => 'training_plan_target_selection',
                        'training_plan_id' => $plan->id,
