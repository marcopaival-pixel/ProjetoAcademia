...
class OrchestratorService
{
    ...
    public function run(User $user, string $message, array $context = []): array
    {
        ...
        try {
            $featureKey = (string) ($context['feature_key'] ?? 'ai_orchestrator');
            $this->governance->assertRoute($featureKey);
            ...
            // 2. Fluxo de Visão (Se houver imagem ou imagens)
            if (!empty($context['image_path']) || !empty($context['image_base64']) || !empty($context['image_url']) || !empty($context['images'])) {
            ...
            if (($result['ok'] ?? false) && in_array($intent, ['training', 'nutrition', 'clinical'], true)) {
                $audit = $this->safetyAuditor->audit($user, $intent, $result['message'] ?? $result, $context);
                $result['safety_audit'] = $audit;

                if (! ($audit['approved'] ?? false)) {
                    $result['ok'] = false;
                    $result['error'] = 'Resposta bloqueada pela auditoria de seguranca.';
                    $result['message'] = $audit['safe_content'] ?: 'A resposta precisa de revisao profissional antes de ser exibida.';
                }
            }

            if (($result['ok'] ?? false) && is_string($result['message'] ?? null)) {
    ...
    private function extractStructuredAction(string $message): array
    {
        $cleanMessage = $message;
        $action = null;

        if (preg_match('/\[NEXBOT_ACTION\](.*?)\[\/NEXBOT_ACTION\]/s', $message, $matches)) {
            $decoded = json_decode(trim($matches[1]), true);
            if (is_array($decoded) && $this->isAllowedAction($decoded)) {
                $action = [
                    'acao' => $decoded['acao'],
                    'dados' => is_array($decoded['dados'] ?? null) ? $decoded['dados'] : [],
                ];
            }

            $cleanMessage = trim(str_replace($matches[0], '', $message));
        }
