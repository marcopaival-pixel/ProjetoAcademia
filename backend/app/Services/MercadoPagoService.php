...
class MercadoPagoService extends BasePaymentGateway implements PaymentGatewayInterface
{
    ...
    public function createCheckoutPreference(string $accessToken, int $userId, string $payerEmail, $plan, ?Coupon $coupon = null): array
    {
        ...
        if ($plan instanceof \App\Models\Plan) {
            ...
            if ($plan === 'monthly') {
                $title = 'NexShape Premium — mensal';
                $price = $isPatientOnly ? $prices['patient_monthly'] : $prices['monthly'];
                $planCode = 'monthly';
            } elseif ($plan === 'yearly') {
                $title = 'NexShape Premium — anual';
                $price = $prices['yearly'];
                $planCode = 'yearly';
            } else {
        ...
        $payload = [
            'items' => [[
                'title' => $title,
                'description' => 'Acesso ao ecossistema NexShape.',
    ...
    public function createAiCreditsCheckoutPreference(string $accessToken, int $userId, int $packageId): array
    {
        ...
        $payload = [
            'items' => [[
                ...
                'description' => "Adição de {$package->credits} créditos para uso de IA no NexShape.",
                'category_id' => 'services',
                'quantity' => 1,
                'currency_id' => 'BRL',
    ...
    public function createPreapprovalSubscription(
        ...
        $reason = $plan === 'yearly'
            ? 'NexShape Premium — assinatura anual'
            : 'NexShape Premium — assinatura mensal';
