...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('nutritionHub', () => ({
            ...
            aiEntryModalOpen: false,
            voiceNoticeOpen: false,
            voiceNoticeTitle: '',
            voiceNoticeMessage: '',
            registrationMode: '',
            openRegisterFlow(mode = '') {
                this.registrationMode = mode;
                this.registerModalOpen = true;
                this.$nextTick(() => { if (window.lucide) window.lucide.createIcons(); });
            },
            chooseRegisterMode(mode) {
                this.registrationMode = mode;
                this.registerModalOpen = false;
                if (mode === 'ai') {
                    this.aiEntryModalOpen = true;
                    this.$nextTick(() => { if (window.lucide) window.lucide.createIcons(); });
                } else if (mode === 'ai_text') {
                    setTimeout(() => {
                        document.querySelector('textarea[x-model="aiInput"]')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        document.querySelector('textarea[x-model="aiInput"]')?.focus();
                    }, 80);
                } else if (mode === 'photo' || mode === 'ai_photo') {
                    this.photoOpen = true;
                } else if (mode === 'voice' || mode === 'ai_voice') {
                    this.startVoiceMealInput();
                } else if (mode === 'search') {
                    setTimeout(() => {
                        document.querySelector('input[name="food_name"]')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        document.querySelector('input[name="food_name"]')?.focus();
                    }, 80);
                } else if (mode === 'manual') {
                    setTimeout(() => {
                        document.querySelector('input[name="calories"]')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        document.querySelector('input[name="calories"]')?.focus();
                    }, 80);
                }
            },
            startVoiceMealInput() {
                const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
                const target = document.querySelector('textarea[x-model="aiInput"]');
                if (!SpeechRecognition) {
                    this.showVoiceNotice(
                        'Voz indisponivel neste navegador',
                        'O reconhecimento de voz nao foi liberado pelo navegador. Use o registro com IA por texto ou abra em um navegador compativel com permissao de microfone.'
                    );
                    setTimeout(() => {
                ...
                recognition.onresult = (event) => {
                    this.aiInput = event.results[0][0].transcript;
                    this.$nextTick(() => {
                        target?.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        target?.focus();
                    });
                ...
                recognition.onerror = () => {
                    this.showVoiceNotice(
                        ...
                        'Verifique a permissao do microfone. Em alguns navegadores, a captura por voz so funciona em HTTPS ou localhost; acessos por IP local, como 192.168.0.109, podem ser bloqueados.'
                    );
                };
                recognition.start();
            },
            showVoiceNotice(title, message) {
                this.voiceNoticeTitle = title;
                this.voiceNoticeMessage = message;
                this.voiceNoticeOpen = true;
                this.$nextTick(() => { if (window.lucide) window.lucide.createIcons(); });
            }
        }));
    });
...
<div x-data="nutritionHub" class="py-8 space-y-8 animate-fade-in max-w-[1400px] mx-auto px-4">
    ...
    @if($tab === 'diary')
    @php
        $consumedKcal = (int)($selectedDateSums->cal ?? 0);
        $remainingKcal = max(0, (int)$targetKcal - $consumedKcal);
        $diaryProgress = (int)min(($consumedKcal / ($targetKcal ?: 1)) * 100, 100);
    @endphp
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start pb-20 animate-fade-in">
        <!-- Diario Header -->
        <div class="lg:col-span-12 bg-zinc-900 border border-zinc-800 rounded-[2rem] p-6 md:p-8 shadow-2xl">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                <div class="space-y-5 flex-1">
                    <div>
                        <h2 class="text-3xl font-black text-white tracking-tighter">Diário Alimentar</h2>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div class="rounded-2xl bg-zinc-950 border border-zinc-800 p-4">
                            <p class="text-xl font-black text-white tabular-nums">{{ number_format($consumedKcal, 0) }}</p>
                            <p class="text-[9px] text-zinc-600 font-black uppercase tracking-widest">kcal</p>
                        </div>
                        <div class="rounded-2xl bg-zinc-950 border border-zinc-800 p-4">
                            <p class="text-xl font-black text-rose-400 tabular-nums">{{ number_format($selectedDateSums->p ?? 0, 0) }}g</p>
                            <p class="text-[9px] text-zinc-600 font-black uppercase tracking-widest">Proteína</p>
                        </div>
                        <div class="rounded-2xl bg-zinc-950 border border-zinc-800 p-4">
                            <p class="text-xl font-black text-emerald-400 tabular-nums">{{ number_format($selectedDateSums->c ?? 0, 0) }}g</p>
                            <p class="text-[9px] text-zinc-600 font-black uppercase tracking-widest">Carbo</p>
                        </div>
                        <div class="rounded-2xl bg-zinc-950 border border-zinc-800 p-4">
                            <p class="text-xl font-black text-amber-400 tabular-nums">{{ number_format($selectedDateSums->f ?? 0, 0) }}g</p>
                            <p class="text-[9px] text-zinc-600 font-black uppercase tracking-widest">Gordura</p>
                        </div>
                    </div>
                    <div>
                        <div class="flex items-center justify-between text-xs font-black uppercase tracking-widest mb-3">
                            <span class="text-zinc-500">Meta: {{ number_format($targetKcal, 0) }} kcal</span>
                            <span class="text-emerald-500">Restam: {{ number_format($remainingKcal, 0) }} kcal</span>
                        </div>
                        <div class="h-3 w-full bg-zinc-950 rounded-full overflow-hidden border border-zinc-800 p-0.5">
                            <div class="h-full bg-emerald-500 rounded-full shadow-[0_0_12px_rgba(16,185,129,0.55)] transition-all duration-700" style="width: {{ $diaryProgress }}%"></div>
                        </div>
                    </div>
                </div>
                <button type="button" @click="openRegisterFlow()" class="w-full lg:w-auto px-8 py-5 rounded-2xl bg-emerald-500 text-zinc-950 text-xs font-black uppercase tracking-[0.2em] shadow-2xl shadow-emerald-500/20 hover:bg-emerald-400 active:scale-95 transition-all flex items-center justify-center gap-3">
                ...
                <div class="group relative bg-zinc-900 border border-zinc-800 rounded-[2rem] overflow-hidden transition-all hover:border-emerald-500/20 shadow-xl">
                    ...
                    <div class="divide-y divide-zinc-900/50">
                        ...
                        <div class="py-10 px-6 text-center">
                            <p class="text-white text-sm font-black">Nenhum alimento registrado.</p>
                            <p class="text-zinc-600 text-[10px] font-black uppercase tracking-widest mt-2">Proteína 0g · Carbo 0g · Gordura 0g</p>
                            <button type="button" @click="openRegisterFlow('manual')" class="mt-5 px-5 py-3 rounded-xl bg-zinc-950 border border-zinc-800 text-emerald-500 text-[10px] font-black uppercase tracking-widest hover:border-emerald-500/50 transition-all">
                                + Adicionar alimento
                            </button>
                        </div>
                        @endforelse
    ...
    <div x-show="registerModalOpen"
         class="fixed inset-0 z-[130] flex items-end sm:items-center justify-center p-4 bg-zinc-950/90 backdrop-blur-xl animate-fade-in"
         style="display: none;"
         @keydown.escape.window="registerModalOpen = false">
        <div class="absolute inset-0" @click="registerModalOpen = false"></div>
        <div class="relative bg-zinc-900 border border-zinc-800 w-full max-w-xl rounded-t-[2rem] sm:rounded-[2.5rem] overflow-hidden shadow-2xl animate-fade-in-up">
            <div class="p-8 border-b border-zinc-800 flex items-center justify-between">
                <div>
                    <h5 class="text-white font-black text-2xl tracking-tighter">Como deseja registrar?</h5>
                    <p class="text-zinc-500 text-[10px] font-black uppercase tracking-widest mt-1">Escolha o fluxo mais rapido para esta refeicao</p>
                </div>
                <button @click="registerModalOpen = false" class="w-10 h-10 rounded-xl bg-zinc-950 border border-zinc-800 flex items-center justify-center text-zinc-500 hover:text-white transition-all">
                    <i data-lucide="x" class="w-5 h-5"></i>
                </button>
            </div>
            <div class="p-6 grid grid-cols-1 gap-3">
                <button type="button" @click="chooseRegisterMode('ai')" class="w-full p-5 rounded-2xl bg-emerald-500 text-zinc-950 border border-emerald-400 hover:bg-emerald-400 transition-all text-left flex items-start gap-4 shadow-2xl shadow-emerald-500/20">
                    <i data-lucide="bot" class="w-6 h-6 mt-1"></i>
                    <span class="flex-1">
                        <span class="inline-flex mb-2 rounded-full bg-zinc-950/15 px-3 py-1 text-[9px] font-black uppercase tracking-widest">Recomendado</span>
                        <span class="block font-black">Registrar com IA</span>
                        <span class="block text-xs font-bold text-zinc-900/80 mt-1">Digite, envie uma foto ou fale para identificacao automatica.</span>
                        <span class="block text-[10px] font-black uppercase tracking-widest text-zinc-900/70 mt-2">Voce possui <span x-text="aiCredits"></span> creditos IA</span>
                    </span>
                </button>
                <button type="button" @click="chooseRegisterMode('photo')" class="w-full p-5 rounded-2xl bg-zinc-950 border border-zinc-800 hover:border-emerald-500/50 transition-all text-left flex items-start gap-4">
                    <i data-lucide="camera" class="w-6 h-6 text-emerald-500"></i>
                    <span>
                        <span class="block text-white font-black">Analisar foto da refeicao</span>
                        <span class="block text-xs text-zinc-500 mt-1">Tire uma foto do prato para reconhecimento dos alimentos.</span>
                        <span class="block text-[10px] text-zinc-600 font-black uppercase tracking-widest mt-2">Consome credito IA apos validacao da imagem</span>
                    </span>
                </button>
                <button type="button" @click="chooseRegisterMode('voice')" class="w-full p-5 rounded-2xl bg-zinc-950 border border-zinc-800 hover:border-emerald-500/50 transition-all text-left flex items-start gap-4">
                    <i data-lucide="mic" class="w-6 h-6 text-emerald-500"></i>
                    <span>
                        <span class="block text-white font-black">Registrar por voz</span>
                        <span class="block text-xs text-zinc-500 mt-1">Conte o que voce comeu em linguagem natural.</span>
                        <span class="block text-[10px] text-zinc-600 font-black uppercase tracking-widest mt-2">Consome credito IA ao processar</span>
                    </span>
                </button>
                <button type="button" @click="chooseRegisterMode('search')" class="w-full p-5 rounded-2xl bg-zinc-950 border border-zinc-800 hover:border-emerald-500/50 transition-all text-left flex items-start gap-4">
                    <i data-lucide="search" class="w-6 h-6 text-emerald-500"></i>
                    <span>
                        <span class="block text-white font-black">Pesquisar alimento</span>
                        <span class="block text-xs text-zinc-500 mt-1">Procure alimentos no banco nutricional.</span>
                        <span class="block text-[10px] text-zinc-600 font-black uppercase tracking-widest mt-2">Nao consome credito IA</span>
                    </span>
                </button>
                <button type="button" @click="chooseRegisterMode('manual')" class="w-full p-5 rounded-2xl bg-zinc-950 border border-zinc-800 hover:border-emerald-500/50 transition-all text-left flex items-start gap-4">
                    <i data-lucide="edit-3" class="w-6 h-6 text-emerald-500"></i>
                    <span>
                        <span class="block text-white font-black">Preencher manualmente</span>
                        <span class="block text-xs text-zinc-500 mt-1">Informe todos os dados da refeicao.</span>
                        <span class="block text-[10px] text-zinc-600 font-black uppercase tracking-widest mt-2">Nao consome credito IA</span>
                    </span>
                </button>
            </div>
    ...
    <!-- AI Entry Modal -->
    <div x-show="aiEntryModalOpen"
         class="fixed inset-0 z-[135] flex items-end sm:items-center justify-center p-4 bg-zinc-950/90 backdrop-blur-xl animate-fade-in"
         style="display: none;"
         @keydown.escape.window="aiEntryModalOpen = false">
        <div class="absolute inset-0" @click="aiEntryModalOpen = false"></div>
        <div class="relative bg-zinc-900 border border-zinc-800 w-full max-w-lg rounded-t-[2rem] sm:rounded-[2.5rem] overflow-hidden shadow-2xl animate-fade-in-up">
            <div class="p-8 border-b border-zinc-800 flex items-center justify-between">
                <div>
                    <h5 class="text-white font-black text-2xl tracking-tighter">Registrar com IA</h5>
                    <p class="text-zinc-500 text-[10px] font-black uppercase tracking-widest mt-1">Voce possui <span x-text="aiCredits"></span> creditos disponiveis</p>
                </div>
                <button @click="aiEntryModalOpen = false" class="w-10 h-10 rounded-xl bg-zinc-950 border border-zinc-800 flex items-center justify-center text-zinc-500 hover:text-white transition-all">
                    <i data-lucide="x" class="w-5 h-5"></i>
                </button>
            </div>
            <div class="p-6 grid grid-cols-1 gap-3">
                <button type="button" @click="aiEntryModalOpen = false; chooseRegisterMode('ai_text')" class="w-full p-5 rounded-2xl bg-zinc-950 border border-zinc-800 hover:border-emerald-500/50 transition-all text-left flex items-start gap-4">
                    <i data-lucide="keyboard" class="w-6 h-6 text-emerald-500"></i>
                    <span>
                        <span class="block text-white font-black">Digitar refeicao</span>
                        <span class="block text-xs text-zinc-500 mt-1">Descreva em linguagem natural. Consome credito IA ao processar.</span>
                    </span>
                </button>
                <button type="button" @click="aiEntryModalOpen = false; chooseRegisterMode('ai_photo')" class="w-full p-5 rounded-2xl bg-zinc-950 border border-zinc-800 hover:border-emerald-500/50 transition-all text-left flex items-start gap-4">
                    <i data-lucide="image-plus" class="w-6 h-6 text-emerald-500"></i>
                    <span>
                        <span class="block text-white font-black">Enviar foto</span>
                        <span class="block text-xs text-zinc-500 mt-1">A foto passa por validacao antes da IA para evitar desperdicio.</span>
                    </span>
                </button>
                <button type="button" @click="aiEntryModalOpen = false; chooseRegisterMode('ai_voice')" class="w-full p-5 rounded-2xl bg-zinc-950 border border-zinc-800 hover:border-emerald-500/50 transition-all text-left flex items-start gap-4">
                    <i data-lucide="mic" class="w-6 h-6 text-emerald-500"></i>
                    <span>
                        <span class="block text-white font-black">Falar</span>
                        <span class="block text-xs text-zinc-500 mt-1">Use voz para preencher a descricao da refeicao.</span>
                    </span>
                </button>
            </div>
        </div>
    ...
    <!-- Premium Voice Notice Modal -->
    <div x-show="voiceNoticeOpen"
         class="fixed inset-0 z-[160] flex items-center justify-center p-4 bg-zinc-950/90 backdrop-blur-2xl animate-fade-in"
         style="display: none;"
         @keydown.escape.window="voiceNoticeOpen = false">
        <div class="absolute inset-0" @click="voiceNoticeOpen = false"></div>
        <div class="relative w-full max-w-md overflow-hidden rounded-[2rem] border border-emerald-500/20 bg-zinc-900 shadow-2xl shadow-emerald-500/10 animate-fade-in-up">
            <div class="absolute -top-24 -right-24 h-48 w-48 rounded-full bg-emerald-500/10 blur-3xl"></div>
            <div class="relative p-8">
                <div class="mb-6 flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        <div class="flex h-14 w-14 items-center justify-center rounded-2xl border border-emerald-500/20 bg-emerald-500/10 text-emerald-500">
                            <i data-lucide="mic-off" class="h-7 w-7"></i>
                        </div>
                        <div>
                            <p class="text-[9px] font-black uppercase tracking-[0.25em] text-emerald-500">NexShape Voice</p>
                            <h5 class="text-xl font-black tracking-tighter text-white" x-text="voiceNoticeTitle"></h5>
                        </div>
                    </div>
                    <button @click="voiceNoticeOpen = false" class="flex h-10 w-10 items-center justify-center rounded-xl border border-zinc-800 bg-zinc-950 text-zinc-500 transition-all hover:text-white">
                        <i data-lucide="x" class="h-5 w-5"></i>
                    </button>
                </div>

                <p class="text-sm font-medium leading-relaxed text-zinc-400" x-text="voiceNoticeMessage"></p>

                <div class="mt-6 rounded-2xl border border-zinc-800 bg-zinc-950 p-5">
                    <p class="text-[10px] font-black uppercase tracking-widest text-zinc-500">Alternativa recomendada</p>
                    <p class="mt-2 text-sm font-bold text-white">Use “Digitar refeição” no Registro com IA. O resultado nutricional continua usando o orquestrador de IA.</p>
                </div>

                <div class="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <button type="button" @click="voiceNoticeOpen = false; chooseRegisterMode('ai_text')" class="rounded-2xl bg-emerald-500 px-5 py-4 text-[10px] font-black uppercase tracking-widest text-zinc-950 transition-all hover:bg-emerald-400 active:scale-95">
                        Digitar refeicao
                    </button>
                    <button type="button" @click="voiceNoticeOpen = false" class="rounded-2xl border border-zinc-800 bg-zinc-950 px-5 py-4 text-[10px] font-black uppercase tracking-widest text-zinc-400 transition-all hover:text-white">
                        Entendi
                    </button>
                </div>
            </div>
