...
<div class="px-6 py-10 mx-auto max-w-6xl animate-fade-in-up" x-data="nexBot()">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-10 h-[75vh]">
        ...
        <div class="lg:col-span-3 flex flex-col bg-zinc-950 border border-zinc-800 rounded-[3.5rem] overflow-hidden shadow-3xl relative">
            <!-- Quota Bar -->
            <div id="chatQuotaBar"
                 class="border-b py-3 px-8 text-[9px] font-black text-center tracking-[0.28em] uppercase shadow-lg"
                 x-show="quota"
                 :class="quota && quota.is_premium ? 'bg-emerald-500/10 border-emerald-500/10 text-emerald-500 shadow-emerald-500/5' : 'bg-amber-500/10 border-amber-500/10 text-amber-400 shadow-amber-500/5'">
...
function nexBot() {
    return {
        ...
        quotaLabel() {
            ...
            if (this.quota.is_premium) {
                const credits = Number(this.quota.remaining_credits || 0);
                return credits > 0
                    ? `ACESSO ELITE ATIVO - ${credits} CREDITOS DE IA DISPONIVEIS`
                    : 'ACESSO ELITE ATIVO - PROCESSAMENTO PRIORITARIO';
            }

            const used = Number(this.quota.daily_user_used || 0);
            const limit = Number(this.quota.daily_user_limit || 0);
            const credits = Number(this.quota.remaining_credits || 0);

            return credits > 0
                ? `ACESSO COM CREDITOS - ${credits} CREDITOS DISPONIVEIS`
                : `PLANO FREE - ${used}/${limit} MENSAGENS USADAS HOJE`;
        },
        ...
        actionLabel(action) {
            const labels = {
                ...
                criar_treino: 'Criar treino',
                ajustar_treino: 'Ajustar treino',
                criar_dieta: 'Criar dieta',
                ajustar_dieta: 'Ajustar dieta'
            };
        ...
        async sendMessage() {
            ...
            try {
                ...
                if (d.ok) {
                    ...
                    this.messages.push({ id: Date.now() + 1, role: 'assistant', message: 'Aviso: ' + (d.error || 'Falha na resposta do NexBot.') });
                ...
                this.messages.push({ id: Date.now() + 1, role: 'assistant', message: 'Erro de conexao com o NexBot. Tente novamente em alguns segundos.' });
            } finally {
                this.loading = false;
                this.scrollToBottom();
                this.$nextTick(() => { lucide.createIcons(); });
