    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="flex flex-col gap-6">
            ...
            <form action="{{ route('progression.plans.store-target-selection') }}"
                    ...
                    <label for="dropzone-file"
                        ...
                        <template x-if="photoPreview">
                            <div class="flex items-center gap-4 px-4 w-full">
                                <img :src="photoPreview" alt="" class="h-16 w-16 rounded-xl object-cover border border-slate-700">
                                <div class="min-w-0 flex-1">
                                    <p class="text-sm font-semibold text-blue-300 truncate" x-text="photoName"></p>
                                    <p class="text-xs text-slate-500 mt-1">Imagem anexada. Ela será salva ao continuar.</p>
                                </div>
                                <button type="button"
                                        @click.prevent="showPhotoPreview = true"
                                        class="shrink-0 px-3 py-2 rounded-xl bg-blue-600/15 border border-blue-500/30 text-blue-300 hover:bg-blue-600 hover:text-white text-[10px] font-black uppercase tracking-widest transition">
                                    Visualizar
                                </button>
                                <button type="button"
                                        @click.prevent="clearPhoto()"
                                        class="shrink-0 w-9 h-9 rounded-xl bg-slate-950 text-slate-400 hover:text-white transition"
                        ...
                        <input id="dropzone-file" x-ref="photoInput" type="file" name="photo" class="hidden" accept="image/png,image/jpeg" @change="handlePhotoSelect($event)">
                    </label>
                    <div x-show="photoValidationStatus !== 'idle'" x-cloak class="mt-4 rounded-xl border p-4"
                         :class="photoValidationStatus === 'rejected' ? 'border-amber-500/30 bg-amber-500/10' : 'border-emerald-500/25 bg-emerald-500/10'">
                        <template x-if="photoValidationStatus === 'validating'">
                            <p class="text-sm font-bold text-blue-300">Validando foto...</p>
                        </template>
                        <template x-if="photoValidationStatus === 'approved'">
                            <div class="space-y-2">
                                <p class="text-sm font-bold text-emerald-300">Foto pronta para análise</p>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-300">
                                    <p>✔ Fotografia detectada</p>
                                    <p>✔ Qualidade aprovada</p>
                                    <p x-show="photoValidation.classification?.view">✔ Tipo: <span x-text="photoLabel()"></span></p>
                                    <p x-show="photoValidation.classification?.framing">✔ Enquadramento: <span x-text="framingLabel()"></span></p>
                                </div>
                                <template x-if="photoValidation.warnings?.length">
                                    <p class="text-xs text-amber-200" x-text="photoValidation.warnings[0]"></p>
                                </template>
                                <template x-if="photoValidation.next_suggestions?.length">
                                    <p class="text-xs text-blue-200" x-text="photoValidation.next_suggestions[0]"></p>
                                </template>
                            </div>
                        </template>
                        <template x-if="photoValidationStatus === 'rejected'">
                            <div>
                                <p class="text-sm font-bold text-amber-200">Encontramos alguns problemas:</p>
                                <ul class="mt-2 space-y-1 text-xs text-amber-100">
                                    <template x-for="message in photoValidationMessages" :key="message">
                                        <li>• <span x-text="message"></span></li>
                                    </template>
                                </ul>
                                <p class="mt-3 text-xs text-slate-400">Escolha outra foto ou remova o anexo para continuar sem foto.</p>
                            </div>
                        </template>
                    </div>

                    <div class="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <template x-for="slot in guidedPhotoSlots" :key="slot.type">
                            <label class="rounded-xl border border-slate-700 bg-slate-900/50 p-3 cursor-pointer hover:border-blue-500/40 transition">
                                <input class="hidden"
                                       type="file"
                                       :name="`photos[${slot.type}]`"
                                       accept="image/png,image/jpeg,image/webp"
                                       @change="handleGuidedPhotoSelect(slot.type, $event)">
                                <div class="flex items-center gap-3">
                                    <div class="h-12 w-12 rounded-lg bg-slate-950 border border-slate-700 overflow-hidden flex items-center justify-center">
                                        <template x-if="guidedPhotos[slot.type]?.preview">
                                            <img :src="guidedPhotos[slot.type].preview" alt="" class="h-full w-full object-cover">
                                        </template>
                                        <template x-if="!guidedPhotos[slot.type]?.preview">
                                            <span class="text-[10px] font-black text-slate-500 uppercase" x-text="slot.short"></span>
                                        </template>
                                    </div>
                                    <div class="min-w-0">
                                        <p class="text-xs font-bold text-slate-200" x-text="slot.label"></p>
                                        <p class="text-[11px] text-slate-500 truncate" x-text="guidedPhotoText(slot.type)"></p>
                                    </div>
                                </div>
                            </label>
                        </template>
                ...
                <button type="submit"
                        :disabled="selected.length === 0 || hasPendingOrRejectedPhotos()"
                        :class="selected.length === 0 || hasPendingOrRejectedPhotos()
        ...
        <div x-show="showPhotoPreview"
             x-cloak
             class="fixed inset-0 z-[110] flex items-center justify-center p-4">
            <div class="absolute inset-0 bg-black/85 backdrop-blur-sm" @click="showPhotoPreview = false"></div>
            <div class="relative w-full max-w-3xl rounded-2xl border border-white/10 bg-zinc-950 shadow-2xl overflow-hidden">
                <div class="flex items-center justify-between px-5 py-4 border-b border-white/10">
                    <p class="min-w-0 truncate text-sm font-bold text-white" x-text="photoName"></p>
                    <button type="button"
                            @click="showPhotoPreview = false"
                            class="ml-4 w-10 h-10 rounded-xl bg-black/40 text-zinc-400 hover:text-white transition"
...
document.addEventListener('alpine:init', () => {
    Alpine.data('bodyTargetSelector', () => ({
        ...
        guidedPhotoSlots: [
            { type: 'back', label: 'Costas', short: 'C' },
            { type: 'right_side', label: 'Lado direito', short: 'LD' },
            { type: 'left_side', label: 'Lado esquerdo', short: 'LE' },
        ],
        guidedPhotos: {},
        ...
        clearPhoto() {
            ...
            this.photoValidationStatus = 'idle';
            this.photoValidation = {};
            this.photoValidationMessages = [];
            this.$refs.photoInput.value = '';
        },

        handleGuidedPhotoSelect(type, event) {
            const file = event.target.files?.[0];
            if (!file || !file.type.match(/^image\/(png|jpeg|webp)$/)) return;

            if (this.guidedPhotos[type]?.preview) {
                URL.revokeObjectURL(this.guidedPhotos[type].preview);
            }

            this.guidedPhotos[type] = {
                name: file.name,
            ...
            this.validateGuidedPhoto(type, file);
        },

        async validatePhoto(file) {
            this.photoValidationStatus = 'validating';
            this.photoValidation = {};
            this.photoValidationMessages = [];
            ...
            try {
                const response = await fetch(`{{ route('progression.plans.validate-target-photo') }}`, {
                    method: 'POST',
                    headers: {
                ...
                this.photoValidation = data;
                this.photoValidationMessages = data.messages || [];
                this.photoValidationStatus = data.approved ? 'approved' : 'rejected';
            } catch (e) {
                this.photoValidationStatus = 'rejected';
                this.photoValidationMessages = ['Não foi possível validar a foto agora. Tente novamente.'];
            }
        },

        async validateGuidedPhoto(type, file) {
            try {
                const data = await this.requestPhotoValidation(file);
                this.guidedPhotos[type] = {
                    ...this.guidedPhotos[type],
                ...
                this.guidedPhotos[type] = {
                    ...this.guidedPhotos[type],
                    status: 'rejected',
                    messages: ['Não foi possível validar a foto agora. Tente novamente.'],
                };
            }
        },

        async requestPhotoValidation(file) {
            const formData = new FormData();
            formData.append('photo', file);
        ...
        guidedPhotoText(type) {
            const photo = this.guidedPhotos[type];
            if (!photo) return 'Adicionar foto';
            if (photo.status === 'validating') return 'Validando...';
            if (photo.status === 'rejected') return photo.messages?.[0] || 'Foto reprovada';
            return photo.name || 'Foto aprovada';
        },

        hasPendingOrRejectedPhotos() {
            if (this.photoValidationStatus === 'validating' || this.photoValidationStatus === 'rejected') {
                return true;
            }

            return Object.values(this.guidedPhotos).some(photo => photo.status === 'validating' || photo.status === 'rejected');
        },
        ...
        photoLabel() {
            ...
            return labels[this.photoValidation.classification?.view] || this.photoValidation.classification?.view;
        },
        ...
        framingLabel() {
            const labels = {
                ...
                face_only: 'Apenas rosto',
                unknown: 'Não identificado',
            };
            return labels[this.photoValidation.classification?.framing] || this.photoValidation.classification?.framing;
        }
    }));
});
</script>
