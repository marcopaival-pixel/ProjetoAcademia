...
<div x-data="{ 
        ...
        updateImagePreviews(event) {
            ...
            this.imagePreviews = Array.from(event.target.files || []).map(file => ({
                name: file.name,
                url: URL.createObjectURL(file),
            }));
        },
        clearImagePreviews(input) {
            this.imagePreviews.forEach(image => URL.revokeObjectURL(image.url));
            this.imagePreviews = [];
    ...
    <div @click.away="open = false" 
        ...
        <form action="{{ route('community.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 relative z-10">
            ...
            <div class="flex items-center justify-between">
                ...
                <button type="button" @click="open = false; clearImagePreviews($refs.imageInput)" class="w-10 h-10 bg-zinc-800 text-zinc-500 hover:text-white rounded-xl transition-all flex items-center justify-center">
            ...
            <div class="space-y-4">
                ...
                <div x-show="imagePreviews.length > 0" x-cloak class="grid gap-3" :class="imagePreviews.length > 1 ? 'grid-cols-2' : 'grid-cols-1'">
                    <template x-for="(image, index) in imagePreviews" :key="image.url">
                        <div class="relative h-40 sm:h-48 overflow-hidden rounded-2xl bg-zinc-950 border border-zinc-800">
                            <img :src="image.url" :alt="image.name" class="w-full h-full object-cover">
                            <button type="button"
                                    @click="const input = $refs.imageInput; const files = Array.from(input.files); files.splice(index, 1); const dt = new DataTransfer(); files.forEach(file => dt.items.add(file)); input.files = dt.files; updateImagePreviews({ target: input });"
                                    class="absolute top-3 right-3 w-8 h-8 rounded-xl bg-zinc-950/80 border border-white/10 text-zinc-300 hover:text-white hover:bg-rose-500 transition-all flex items-center justify-center">
                ...
                <div class="flex items-center gap-4">
                    <label class="flex-1 flex items-center justify-center gap-3 px-6 py-4 bg-zinc-950 border border-zinc-800 rounded-2xl text-zinc-500 hover:text-white hover:border-emerald-500/30 transition-all cursor-pointer group/input">
                        ...
                        <input x-ref="imageInput" @change="updateImagePreviews($event)" type="file" name="images[]" multiple accept="image/*" class="hidden">
                    </label>
