...
@section('title', 'Editar Artigo — Base de Conhecimento')
...
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-3xl font-black text-white">Editar Artigo</h1>
    </div>

    <form action="{{ route('admin.kb.update', $article->id) }}" method="POST" class="bg-zinc-900/60 border border-white/5 rounded-[2.5rem] p-10 space-y-8 shadow-2xl">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="md:col-span-2">
                <label class="text-[10px] font-black text-zinc-500 uppercase tracking-widest block mb-2">Título do Artigo</label>
                <input type="text" name="titulo" required value="{{ old('titulo', $article->titulo) }}" class="w-full bg-zinc-950 border border-white/10 rounded-2xl px-6 py-4 text-white focus:border-blue-500 transition-all font-bold text-lg">
            </div>
            ...
            <div>
                ...
                <select name="categoria_id" required class="w-full bg-zinc-950 border border-white/10 rounded-2xl px-6 py-4 text-white focus:border-blue-500 transition-all font-bold">
                    @foreach($categories as $cat)
                        <option value="{{ $cat->id }}" {{ $article->categoria_id == $cat->id ? 'selected' : '' }}>{{ $cat->nome }} ({{ $cat->tipo_usuario }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="tipo_usuario" required class="w-full bg-zinc-950 border border-white/10 rounded-2xl px-6 py-4 text-white focus:border-blue-500 transition-all font-bold">
                    @foreach($types as $type)
                        <option value="{{ $type }}" {{ $article->tipo_usuario == $type ? 'selected' : '' }}>{{ $type }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="conteudo" required class="w-full bg-zinc-950 border border-white/10 rounded-2xl px-6 py-4 text-white focus:border-blue-500 transition-all min-h-[400px] font-medium leading-relaxed">{{ old('conteudo', $article->conteudo) }}</textarea>
        ...
        <div class="flex items-center gap-3 bg-zinc-950/50 p-6 rounded-2xl border border-white/5">
            <input type="checkbox" name="ativo" value="1" {{ $article->ativo ? 'checked' : '' }} class="w-6 h-6 rounded-lg bg-zinc-900 border-white/10 text-blue-600 focus:ring-0">
        ...
        <div class="pt-6 flex gap-4">
            ...
            <button type="submit" class="flex-[2] py-5 bg-blue-600 text-white font-black rounded-2xl hover:bg-blue-500 transition-all text-xs uppercase tracking-[0.2em] shadow-xl shadow-blue-500/20">Salvar Alterações</button>
        </div>
    </form>
</div>
@endsection
