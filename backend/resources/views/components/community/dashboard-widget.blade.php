...
@php
    $communityInitialCounts = $posts->mapWithKeys(fn ($post) => [
        $post->id => [
...
<div class="lg:col-span-12 group bg-zinc-900/50 backdrop-blur-3xl border border-white/5 p-12 rounded-[4rem] relative overflow-hidden shadow-[0_0_80px_rgba(0,0,0,0.5)] transition-all hover:border-emerald-500/20" x-data='communityWidget(@json($communityInitialCounts))' x-init="startSync()">
        ...
        @forelse($posts as $post)
            <div class="bg-zinc-950/40 backdrop-blur-xl border border-white/5 p-8 rounded-[2.5rem] space-y-6 hover:border-emerald-500/30 hover:bg-zinc-950/60 transition-all duration-500 flex flex-col group/post relative overflow-hidden">
                ...
                @if($post->media->count() > 0)
                    <div class="relative aspect-video rounded-3xl overflow-hidden border border-white/5 bg-zinc-900/70 group-hover/post:border-emerald-500/20 transition-all duration-500 shadow-2xl">
                        ...
                        @else
                            <button type="button" @click="imageLightbox = '{{ $firstMedia->url }}'" class="w-full h-full block group/media">
                                <img src="{{ $firstMedia->url }}" class="w-full h-full object-contain grayscale-[20%] group-hover/post:grayscale-0 transition-all duration-700 group-hover/media:scale-105">
                            </button>
                        @endif
                ...
                <div class="flex items-center justify-between pt-6 border-t border-white/5 relative z-10">
                    <div class="flex items-center gap-5">
                        @php
                            $userReacted = auth()->check() && $post->reactions->contains(fn ($reaction) => (int) $reaction->user_id === (int) auth()->id() && $reaction->emoji === '❤️');
                        @endphp
                        <button type="button" @click="reactPost({{ $post->id }})" class="flex items-center gap-2 {{ $userReacted ? 'text-red-400' : 'text-zinc-500 hover:text-red-400' }} transition-colors">
                            <i data-lucide="heart" class="w-4 h-4 {{ $userReacted ? 'fill-red-500/30' : '' }}"></i>
                                    <span class="text-[11px] font-black tabular-nums" x-text="counts[{{ $post->id }}]?.reactions ?? {{ $post->reactions->count() }}"></span>
                                </button>
                        <button type="button" @click="openComment({{ $post->id }})" class="flex items-center gap-2 text-zinc-500 hover:text-emerald-400 transition-colors">
                            <i data-lucide="message-square" class="w-4 h-4"></i>
                            <span class="text-[11px] font-black tabular-nums" x-text="counts[{{ $post->id }}]?.comments ?? {{ $post->comments->count() }}"></span>
                        </button>
                    </div>
    ...
    <div x-show="commentBox.open" x-cloak class="fixed inset-0 z-[225] bg-zinc-950/85 backdrop-blur-sm flex items-center justify-center p-4" x-transition.opacity @keydown.escape.window="commentBox.open = false">
        <button type="button" class="absolute inset-0" @click="commentBox.open = false" aria-label="Fechar comentário"></button>
        <div class="relative w-full max-w-md bg-zinc-900 border border-emerald-500/20 rounded-[2rem] p-6 shadow-2xl">
            <div class="mb-5">
                <p class="text-[10px] text-emerald-400 font-black uppercase tracking-[0.25em]">Comentar publicação</p>
                <h3 class="text-xl text-white font-black uppercase tracking-tight mt-1">Deixe sua mensagem</h3>
            </div>

            <form @submit.prevent="submitComment()" class="space-y-4">
                @guest
                    <input type="text" x-model="commentBox.name" placeholder="Seu nome" class="w-full bg-zinc-950 border border-zinc-800 rounded-2xl px-5 py-4 text-sm text-white outline-none focus:border-emerald-500/40">
                @endguest
                <textarea x-model="commentBox.content" required maxlength="500" rows="4" placeholder="Escreva um comentário..." class="w-full bg-zinc-950 border border-zinc-800 rounded-2xl px-5 py-4 text-sm text-white outline-none focus:border-emerald-500/40 resize-none"></textarea>
                <div class="grid grid-cols-2 gap-3">
                    <button type="button" @click="commentBox.open = false" class="py-4 bg-zinc-950 border border-zinc-800 text-zinc-300 hover:text-white rounded-2xl text-xs font-black uppercase tracking-widest transition-all">Cancelar</button>
                    <button type="submit" class="py-4 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 rounded-2xl text-xs font-black uppercase tracking-widest transition-all">Comentar</button>
                </div>
            </form>
        </div>
    </div>
...
@once
    @push('scripts')
        <script>
            function communityWidget(initialCounts = {}) {
                return {
                    loading: false,
                    imageLightbox: null,
                    commentBox: { open: false, postId: null, name: '', content: '' },
                    ...
                    reactPost(postId) {
                        fetch(`/community/react/post/${postId}`, {
                            ...
                            .then(data => {
                                this.counts[postId] = {
                                    reactions: data.reactions_count ?? this.counts[postId]?.reactions ?? 0,
                                    comments: data.comments_count ?? this.counts[postId]?.comments ?? 0,
                                };
                            });
                    },
                    openComment(postId) {
                        this.commentBox = { open: true, postId, name: '', content: '' };
                    },
                    submitComment() {
                        if (!this.commentBox.postId || !this.commentBox.content.trim()) return;

                        fetch(`/community/post/${this.commentBox.postId}/comment`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            ...
                            body: JSON.stringify({
                                content: this.commentBox.content,
                                visitor_name: this.commentBox.name || 'Visitante',
                            })
                        })
                            .then(response => response.json())
                            .then(data => {
                                this.counts[this.commentBox.postId] = {
                                    reactions: data.reactions_count ?? this.counts[this.commentBox.postId]?.reactions ?? 0,
                                    comments: data.comments_count ?? this.counts[this.commentBox.postId]?.comments ?? 0,
                                };
                                this.commentBox.open = false;
                            });
                    },
                    startSync() {
                        this.syncCounts();
