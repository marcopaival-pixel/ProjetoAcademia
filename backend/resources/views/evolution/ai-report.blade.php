...
@section('title', 'Relatório inteligente de evolução visual — NexShape')
...
<div class="py-10 space-y-8 animate-fade-in-up max-w-[1200px] mx-auto px-6 relative z-10 pb-32">
    ...
    <div id="print-section" class="space-y-6">
        ...
        @if(isset($reportData['status']))
            @php
                $confidence = (float) ($reportData['confianca_geral'] ?? 0);
                $confidenceLabel = match (true) {
                    $confidence >= 0.8 => 'Alta confiança',
                    $confidence >= 0.6 => 'Confiança moderada',
                    $confidence > 0 => 'Baixa confiança',
                    default => 'Não analisável',
                };
            @endphp

            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="elite-card p-8 lg:col-span-2">
                    <div class="flex items-center gap-3 mb-4">
                        <i data-lucide="shield-check" class="w-6 h-6 text-emerald-500"></i>
                        <h2 class="text-sm font-black text-white uppercase tracking-widest">{{ $reportData['nome'] ?? 'Relatório inteligente de evolução visual' }}</h2>
                    </div>
                    <p class="text-zinc-300 text-base leading-relaxed">Status: <span class="text-emerald-400 font-black">{{ str_replace('_', ' ', $reportData['status']) }}</span></p>
                    <p class="text-zinc-500 text-sm mt-3">
                        Registro anterior: {{ $reportData['periodo_comparado']['registro_anterior'] ?? 'não disponível' }} ·
                        Registro atual: {{ $reportData['periodo_comparado']['registro_atual'] ?? 'não disponível' }}
                    </p>
                </div>
                <div class="elite-card p-8 text-center">
                    <p class="text-xs font-black text-zinc-400 uppercase tracking-widest mb-2">{{ $confidenceLabel }}</p>
                    <div class="text-6xl font-black text-white tracking-tighter">{{ (int) round($confidence * 100) }}<span class="text-2xl text-emerald-500">%</span></div>
                    <p class="text-xs text-zinc-500 mt-3 uppercase tracking-widest font-bold">Confiança geral</p>
                </div>
            </div>

            @if(!empty($reportData['alertas']))
                <div class="elite-card p-8 border-l-4 border-l-amber-500">
                    <h3 class="text-xs font-black text-amber-300 uppercase tracking-widest mb-4">Alertas</h3>
                    <ul class="space-y-2">
                        @foreach($reportData['alertas'] as $alerta)
                            <li class="text-sm text-zinc-300">{{ $alerta }}</li>
                        @endforeach
                    </ul>
                </div>
            @endif
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="elite-card p-8">
                    <h3 class="text-xs font-black text-emerald-500 uppercase tracking-widest mb-5">Dados confirmados</h3>
                    <div class="space-y-4">
                        @forelse($reportData['dados_confirmados'] ?? [] as $item)
                            <div class="border-b border-white/5 pb-3">
                                <p class="text-white font-bold">{{ $item['metrica'] }}</p>
                                <p class="text-sm text-zinc-400">
                                    Atual: {{ $item['atual'] }}
                                    @if(isset($item['anterior'])) · Anterior: {{ $item['anterior'] }} @endif
                                    @if(isset($item['variacao_absoluta'])) · Variação: {{ $item['variacao_absoluta'] }} @endif
                                    @if(isset($item['variacao_percentual'])) ({{ $item['variacao_percentual'] }}%) @endif
                                </p>
                                <p class="text-[10px] text-zinc-600 uppercase tracking-widest mt-1">Fonte: {{ $item['fonte'] }} · Confiança {{ (int) (($item['confianca'] ?? 0) * 100) }}%</p>
                            </div>
                        @empty
                            <p class="text-zinc-500 text-sm">Nenhum dado objetivo disponível para cálculo.</p>
                        @endforelse
                    </div>
                </div>

                <div class="elite-card p-8">
                    <h3 class="text-xs font-black text-emerald-500 uppercase tracking-widest mb-5">Comparação visual</h3>
                    <div class="space-y-4">
                        @forelse($reportData['observacoes_visuais'] ?? [] as $item)
                            <div class="border-b border-white/5 pb-3">
                                <p class="text-zinc-300 text-sm leading-relaxed">{{ $item['texto'] }}</p>
                                <p class="text-[10px] text-zinc-600 uppercase tracking-widest mt-2">Fonte: {{ $item['fonte'] }} · Confiança {{ (int) (($item['confianca'] ?? 0) * 100) }}%</p>
                                @if(!empty($item['limitacao']))
                                    <p class="text-[11px] text-amber-300 mt-1">{{ $item['limitacao'] }}</p>
                                @endif
                            </div>
                        @empty
                            <p class="text-zinc-500 text-sm">Não foi possível confirmar mudança visual com as evidências atuais.</p>
                        @endforelse
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="elite-card p-8">
                    <h3 class="text-xs font-black text-white uppercase tracking-widest mb-4">Qualidade das fotos</h3>
                    <pre class="text-xs text-zinc-400 whitespace-pre-wrap">{{ json_encode($reportData['qualidade_fotos'] ?? [], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                </div>
                <div class="elite-card p-8">
                    <h3 class="text-xs font-black text-white uppercase tracking-widest mb-4">Limitações</h3>
                    <ul class="space-y-2">
                        @foreach($reportData['limitacoes'] ?? [] as $item)
                            <li class="text-sm text-zinc-400">{{ $item }}</li>
                        @endforeach
                    </ul>
                </div>
                <div class="elite-card p-8">
                    <h3 class="text-xs font-black text-white uppercase tracking-widest mb-4">Orientações para o próximo registro</h3>
                    <ul class="space-y-2">
                        @foreach($reportData['recomendacoes'] ?? [] as $item)
                            <li class="text-sm text-zinc-400">{{ $item }}</li>
                        @endforeach
                    </ul>
                </div>
            ...
            <div class="elite-card p-8 border-l-4 border-l-emerald-500">
                <h3 class="text-xs font-black text-white uppercase tracking-widest mb-3">Auditoria anti-alucinação</h3>
                <p class="text-sm {{ $reportData['auditoria_aprovada'] ? 'text-emerald-300' : 'text-amber-300' }}">{{ $reportData['auditoria_aprovada'] ? 'Auditoria aprovada' : 'Auditoria reprovada ou limitada' }}</p>
                <p class="text-[10px] text-zinc-600 uppercase tracking-widest mt-2">Modelo: {{ $reportData['auditoria']['modelo'] ?? 'não informado' }}</p>
            </div>

        @elseif(isset($reportData['diagnostico']))
