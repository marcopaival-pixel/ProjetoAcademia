...
<div class="py-10 space-y-12 animate-fade-in-up max-w-[1400px] mx-auto px-6 relative z-10 pb-32">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-8 pb-4 border-b border-zinc-900">
        ...
        <div class="flex items-center gap-4">
            ...
            <a href="{{ ($isPremium && $hasCompleteEvolutionRecord) ? route('evolution.ai-report') : '#' }}" {{ !$isPremium ? 'data-premium-locked' : '' }} @if($isPremium && !$hasCompleteEvolutionRecord) aria-disabled="true" title="Complete um registro para gerar o relatório." @endif class="px-6 py-5 {{ ($isPremium && $hasCompleteEvolutionRecord) || !$isPremium ? 'bg-zinc-900 text-zinc-300 hover:text-white hover:border-zinc-700' : 'bg-zinc-900/60 text-zinc-600 pointer-events-none cursor-not-allowed' }} border border-zinc-800 font-black text-xs rounded-2xl transition-all shadow-xl flex items-center gap-2 uppercase tracking-widest">
                <i data-lucide="sparkles" class="w-4 h-4 text-emerald-500"></i>
                <span class="hidden md:inline">Relatório IA</span>
            </a>
            <button onclick="openEvolutionUpload()" class="px-8 py-5 bg-emerald-500 text-zinc-950 font-black text-xs rounded-2xl hover:bg-emerald-400 transition-all shadow-xl shadow-emerald-500/10 flex items-center gap-3 uppercase tracking-widest">
                <i data-lucide="camera" class="w-4 h-4"></i>
                Novo registro
            </button>
        </div>
    </div>
    @if($isPremium && !$hasCompleteEvolutionRecord)
        <p class="text-right text-[10px] text-zinc-600 font-black uppercase tracking-widest -mt-8">Complete um registro para gerar o relatório.</p>
    @endif

    <!-- Guided Photo Checklist -->
    <div class="glass-card rounded-[2rem] p-6 border border-emerald-500/10">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
            <div>
                <h2 class="text-sm font-black text-white uppercase tracking-widest">Fotos de hoje</h2>
                <p class="text-xs text-zinc-500 mt-1">Complete os ângulos para formar um registro completo.</p>
            </div>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mt-5">
            @foreach($guidedPhotoChecklist as $item)
                <button type="button" onclick="openEvolutionUpload('{{ now()->toDateString() }}', '{{ $item['type'] }}')" class="text-left rounded-2xl border p-4 {{ $item['sent'] ? 'border-emerald-500/25 bg-emerald-500/10' : 'border-zinc-800 bg-zinc-950/50 hover:border-emerald-500/30' }} transition-all">
                    <div class="flex items-center justify-between gap-3">
                        <span class="text-[10px] font-black uppercase tracking-widest {{ $item['sent'] ? 'text-emerald-300' : 'text-zinc-500' }}">{{ $item['label'] }}</span>
                        <span class="w-7 h-7 rounded-lg flex items-center justify-center {{ $item['sent'] ? 'bg-emerald-500 text-zinc-950' : 'bg-zinc-900 text-zinc-600 border border-zinc-800' }}">
                            @if($item['sent'])
                    ...
                    <p class="text-[10px] mt-2 {{ $item['sent'] ? 'text-emerald-400/80' : 'text-zinc-600' }}">{{ $item['sent'] ? 'Concluído' : 'Pendente' }}</p>
                </button>
            @endforeach
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-4 gap-12">
        ...
        <div class="xl:col-span-3 space-y-12">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 {{ !$isPremium ? 'blur-sm select-none pointer-events-none' : '' }}">
                    ...
                    <div class="glass-card rounded-[2.5rem] overflow-hidden p-4 space-y-4 hover:border-emerald-500/20 transition-all">
                        <div class="flex items-center justify-between px-2">
                            <span class="text-[9px] font-black uppercase tracking-widest text-zinc-400">Vista {{ [
                                'front' => 'Frontal',
            ...
            <div class="space-y-6">
                <div class="flex items-center justify-between px-2">
                    <h2 class="text-xs font-black uppercase tracking-[0.2em] text-zinc-500">Sessões Corporais</h2>
                    <span class="text-[9px] font-bold text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-full uppercase tracking-widest">Frente • Costas • Laterais</span>
                </div>
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    @foreach($photoSessions as $session)
                        <div class="glass-card rounded-[2rem] p-5 space-y-4" x-data="{ analysisOpen: false, analysisLoading: false, analysis: null, analysisCached: false, analysisError: '' }">
                            <div class="flex items-center justify-between gap-4">
                                <div>
                                    <p class="text-sm font-black text-white tabular-nums">{{ $session['label'] }}</p>
                                    <div class="flex flex-wrap items-center gap-2 mt-1">
                                        <p class="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{{ $session['completion'] }}/4 ângulos capturados</p>
                                        @php($tone = $session['consistency']['tone'])
                                        <span class="px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest border {{
                                            $tone === 'emerald' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' :
                                            ($tone === 'blue' ? 'bg-blue-500/10 border-blue-500/20 text-blue-400' : 'bg-amber-500/10 border-amber-500/20 text-amber-400')
                                        }}">
                                            {{ $session['consistency']['label'] }}
                                        </span>
                                    </div>
                                </div>
                                @if($session['completion'] < 4)
                                    @php($missingType = collect(['front', 'back', 'right_side', 'left_side'])->first(fn ($type) => empty($session['photos'][$type])))
                                    <button type="button"
                                            onclick="openEvolutionUpload('{{ $session['date'] }}', '{{ $missingType }}')"
                                            class="px-4 py-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-zinc-950 transition-all text-[9px] font-black uppercase tracking-widest">
                                        Completar Sessão
                                    </button>
                                @else
                                    <div class="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 flex items-center justify-center text-xs font-black">
                                        {{ $session['completion'] }}
                                    </div>
                                @endif
                            </div>
                            <div class="grid grid-cols-4 gap-2">
                                @foreach([
                                    ...
                                    @php($sessionPhoto = $session['photos'][$type] ?? null)
                                    <div class="space-y-2">
                                        <div class="aspect-[3/4] rounded-xl overflow-hidden bg-zinc-950 border {{ $sessionPhoto ? 'border-emerald-500/30' : 'border-zinc-800' }}">
                                            @if($sessionPhoto)
                                                <img src="{{ asset('storage/' . $sessionPhoto->photo_path) }}" alt="{{ $label }}" class="w-full h-full object-cover">
                                            @else
                                        ...
                                        <p class="text-[9px] font-black uppercase tracking-widest text-center {{ $sessionPhoto ? 'text-emerald-400' : 'text-zinc-600' }}">{{ $label }}</p>
                                    </div>
                                @endforeach
                            </div>
                            @if(in_array($session['consistency']['label'], ['Completa', 'Parcial'], true))
                                <button type="button"
                                        @click="analyzeEvolutionSession('{{ $session['date'] }}', $data)"
                                        class="w-full py-3 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-400 hover:bg-blue-500 hover:text-white transition-all text-[9px] font-black uppercase tracking-widest">
                                    <span x-show="!analysisLoading">Analisar Sessão</span>
                                    <span x-show="analysisLoading">Analisando...</span>
                                </button>
                                <div x-show="analysisOpen" x-cloak class="rounded-2xl border border-blue-500/20 bg-blue-500/10 p-4 space-y-3">
                                    <template x-if="analysisError">
                                        <p class="text-xs text-amber-200" x-text="analysisError"></p>
                                    </template>
                                    <template x-if="analysis">
                                        <div class="space-y-3">
                                            <p class="text-xs font-bold text-white leading-relaxed" x-text="analysis.summary || analysis.message || 'Análise concluída.'"></p>
                                            <p x-show="analysisCached" class="text-[9px] font-black uppercase tracking-widest text-blue-300">Resultado salvo reutilizado</p>
                                            <template x-if="analysis.next_recommendations?.length">
                                                <div>
                                                    <p class="text-[9px] font-black uppercase tracking-widest text-blue-300">Próximas recomendações</p>
                                                    <ul class="mt-2 space-y-1 text-xs text-zinc-300">
                                                        <template x-for="item in analysis.next_recommendations" :key="item">
            ...
            @empty
                <div class="p-20 text-center bg-zinc-900/50 border border-zinc-800 rounded-[3rem] max-w-2xl mx-auto mt-20">
                    ...
                    <p class="text-zinc-500 text-sm mb-8 font-medium">Você ainda não criou um registro de evolução. Comece com as fotos de frente, costas e laterais.</p>
                    <button onclick="openEvolutionUpload()" class="px-8 py-4 bg-emerald-500 text-zinc-950 font-black rounded-2xl hover:bg-emerald-400 transition-all text-xs tracking-widest uppercase shadow-xl shadow-emerald-500/10">
...
<!-- Upload Modal -->
<div id="uploadModal" x-data="evolutionUploadValidator()" class="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-zinc-950/95 backdrop-blur-xl hidden animate-fade-in">
            ...
            <div class="space-y-6">
                <div class="grid grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input id="evolutionUploadDate" type="date" name="registered_date" class="w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-5 text-white text-xs font-black focus:border-emerald-500/50 transition-colors shadow-inner uppercase" value="{{ date('Y-m-d') }}" required>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[9px] uppercase font-black text-zinc-500 tracking-widest ml-2">Ângulo/Tipo</label>
                        <select id="evolutionUploadType" name="type" class="w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-5 text-white text-xs font-black focus:border-emerald-500/50 transition-colors appearance-none shadow-inner uppercase tracking-widest cursor-pointer">
                ...
                <div class="space-y-2">
                    ...
                    <input x-ref="photoInput" @change="handlePhotoSelect($event)" type="file" name="photo" accept="image/png,image/jpeg,image/webp" class="w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-4 text-zinc-600 text-[10px] font-black focus:border-emerald-500/50 transition-colors file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-[9px] file:font-black file:bg-zinc-800 file:text-emerald-500 file:uppercase file:tracking-widest cursor-pointer" required>
                    <div x-show="photoPreview" x-cloak class="mt-4 rounded-2xl border border-zinc-800 bg-zinc-950/70 p-3 flex items-center gap-3">
                        <img :src="photoPreview" alt="" class="w-16 h-16 rounded-xl object-cover border border-zinc-800">
                        <div class="min-w-0 flex-1">
                            <p class="text-xs font-black text-white truncate" x-text="photoName"></p>
                            <p class="text-[10px] font-bold mt-1" :class="validationStatus === 'approved' ? 'text-emerald-400' : (validationStatus === 'rejected' ? 'text-amber-300' : 'text-blue-300')" x-text="validationText()"></p>
                        </div>
                        <button type="button" @click="showPhotoPreview = true" class="px-3 py-2 rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[9px] font-black uppercase tracking-widest hover:bg-emerald-500 hover:text-zinc-950 transition">Ver</button>
                    </div>
                    <div x-show="validationStatus === 'rejected'" x-cloak class="mt-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
                        <p class="text-xs font-black text-amber-200 uppercase tracking-widest">Problemas encontrados</p>
                        <ul class="mt-2 space-y-1 text-xs text-amber-100">
                            <template x-for="message in validationMessages" :key="message">
                                <li>• <span x-text="message"></span></li>
                            </template>
                        </ul>
                    </div>
                    <div x-show="validationStatus === 'approved'" x-cloak class="mt-3 rounded-xl border border-emerald-500/25 bg-emerald-500/10 p-4">
                        <p class="text-xs font-black text-emerald-300 uppercase tracking-widest">Foto aprovada para evolução</p>
                        <p class="text-[11px] text-zinc-400 mt-1" x-text="validationSuggestion()"></p>
                    </div>
                </div>
            </div>
            <div class="pt-6">
                <button type="submit"
                        :disabled="validationStatus === 'validating' || validationStatus === 'rejected'"
                        :class="validationStatus === 'validating' || validationStatus === 'rejected' ? 'opacity-40 cursor-not-allowed' : 'hover:bg-emerald-400 active:scale-95'"
                        class="w-full py-6 bg-emerald-500 text-zinc-950 font-black uppercase text-[10px] tracking-[0.3em] rounded-3xl transition-all shadow-2xl">
                    REGISTRAR AGORA
                </button>
            </div>
        </form>
    </div>
    <div x-show="showPhotoPreview" x-cloak class="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/85" @click.self="showPhotoPreview = false">
        <div class="w-full max-w-3xl rounded-3xl overflow-hidden border border-white/10 bg-zinc-950">
            <div class="flex items-center justify-between px-5 py-4 border-b border-white/10">
                <p class="text-sm font-bold text-white truncate" x-text="photoName"></p>
                <button type="button" @click="showPhotoPreview = false" class="w-10 h-10 rounded-xl bg-black/40 text-zinc-400 hover:text-white">
                    <i data-lucide="x" class="w-5 h-5 mx-auto"></i>
                </button>
            </div>
            <div class="bg-black flex items-center justify-center">
                <img :src="photoPreview" alt="" class="max-h-[70vh] w-auto max-w-full object-contain">
            </div>
        </div>
    </div>
</div>
...
<script>
    function evolutionUploadValidator() {
        return {
            photoName: '',
            photoPreview: '',
            ...
            async validatePhoto(file) {
                ...
                try {
                    const response = await fetch(`{{ route('evolution.validate-photo') }}`, {
                        method: 'POST',
                        headers: {
                    ...
                    this.validationStatus = data.approved ? 'approved' : 'rejected';
                } catch (e) {
                    this.validationStatus = 'rejected';
                    this.validationMessages = ['Não foi possível validar a foto agora. Tente novamente.'];
                }
            },

            validationText() {
                if (this.validationStatus === 'validating') return 'Validando foto...';
                if (this.validationStatus === 'rejected') return 'Foto reprovada';
                if (this.validationStatus === 'approved') return 'Foto aprovada';
                return '';
            },

            validationSuggestion() {
                return this.validation.next_suggestions?.[0]
                    || this.validation.warnings?.[0]
                    || 'Pronta para entrar no histórico visual.';
            },
        };
    }

    function openEvolutionUpload(date = null, type = null) {
        const modal = document.getElementById('uploadModal');
        const dateInput = document.getElementById('evolutionUploadDate');
        const typeInput = document.getElementById('evolutionUploadType');
    ...
    async function analyzeEvolutionSession(date, state) {
        state.analysisOpen = true;
        state.analysisLoading = true;
        state.analysisError = '';

        try {
            const response = await fetch(`{{ route('evolution.analyze-session') }}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Accept': 'application/json',
            ...
            const data = await response.json();
            if (!response.ok) {
                throw new Error(data.error || 'Não foi possível analisar esta sessão.');
            }

            state.analysis = data.analysis || {};
            state.analysisCached = Boolean(data.cached);
        } catch (error) {
            state.analysisError = error.message || 'Não foi possível analisar esta sessão.';
        } finally {
